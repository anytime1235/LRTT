...
def main():
    ...
    general_parser.add_argument('--baseline_mode', type=str, default="analog",
    ...
    print(f"Baseline Mode: {general_args.baseline_mode}")
    print(f"Analog Device: {general_args.analog_device}")
    ...
    if general_args.analog_device == "tikitaka":
        ...
        exclude.append("qa_outputs")  # Always exclude qa_outputs (classifier)
        ...
        # Freeze non-target layers (only train target modules and qa_outputs)
        for name, param in model.named_parameters():
            is_target = any(t in name for t in target_modules)
            if is_target or "qa_outputs" in name:
    ...
    if training_args.do_train:
        ...
        checkpoint_path = os.path.join(training_args.output_dir, "saved_chkpt.pt")
        ...
        # Save initial checkpoint for eval-only mode (needed for drift evaluation reset)
        if general_args.baseline_mode == "analog":
            os.makedirs(training_args.output_dir, exist_ok=True)
            checkpoint_path = os.path.join(training_args.output_dir, "saved_chkpt.pt")
            torch_save(model.state_dict(), checkpoint_path)
            print(f"Saved initial checkpoint for eval-only mode: {checkpoint_path}")

    if training_args.do_eval:
        if general_args.baseline_mode == "digital":
            # Digital mode - simple evaluation without drift
            logger.info("*** Evaluate (Digital Baseline) ***")
            ...
            if general_args.tikitaka_enable_drift:
                ...
                for drift in drift_values:
                    ...
                    for i in range(general_args.num_evaluation_repetition):
                        ...
                        checkpoint_path = os.path.join(training_args.output_dir, "saved_chkpt.pt")
                        model.load_state_dict(torch.load(checkpoint_path, weights_only=False), load_rpu_config=False)
                    ...
                    logger.info(f"Mean metrics for TikiTaka drift = {drift}: {mean_metrics}")
                    logger.info(f"Standard deviation metrics for TikiTaka drift = {drift}: {std_metrics}")
            ...
            device_type = general_args.analog_device.upper()
            print(f"\n{'='*50}")
            print(f"EVALUATING {device_type} ANALOG MODEL WITH DRIFT/RETENTION")
            print(f"{'='*50}\n")
            ...
            for drift in drift_values:
                ...
                for i in range(general_args.num_evaluation_repetition):
                    ...
                    if general_args.analog_device == "sixt1c":
                        ...
                        # Step 1: Apply PCM drift to base_layer
                        apply_pcm_drift_to_base_layer(model, drift)
                        # Step 2: Apply sixt1c retention to lora_A/lora_B
                        apply_sixt1c_lora_retention(model, drift)
                    else:
                        print(
                            f"Testing with drift_analog_weights = {drift} (Repetition {i + 1}/{general_args.num_evaluation_repetition})")
                        # Use PCM drift for other analog devices
