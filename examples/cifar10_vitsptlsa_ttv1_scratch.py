...
"""aihwkit example: ViT with SPT+LSA for CIFAR10 using TikiTaka TTv1 layers.
...
RESULTS = os.path.join(os.getcwd(), "results", "VITSPTLSA_TTV1_SCRATCH")
os.makedirs(RESULTS, exist_ok=True)
WEIGHT_PATH = os.path.join(RESULTS, "cifar10_vitsptlsa_ttv1_scratch_model_weight.pth")
...
# TikiTaka TTv1 configuration parameters
TRANSFER_EVERY = 1.0  # Transfer frequency (every mini-batch)
UNITS_IN_MBATCH = True  # Whether to use mini-batch units
...
def create_ttv1_config():
    """Create TikiTaka TTv1 configuration for linear/conv layers."""

    # Use IdealizedPresetDevice for both devices in the transfer compound
    unit_devices = [
    ...
    print("Using TikiTaka TTv1 with IdealizedPresetDevice (dw_min=0.0002)")
    ...
    # Create the config using TikiTakaIdealizedPreset as base and modify
...
def create_model():
    """Create ViT-SPT-LSA model with TikiTaka TTv1 layers."""
    ...
    print(f"\nCreated ViT-SPT-LSA model with TikiTaka TTv1:")
    ...
    print(f"  Linear/Conv layers: {num_linear} (18 total: 16 TTv1 + 2 digital [SPT, head])")
...
def main():
    """Train ViT-SPT-LSA with TikiTaka TTv1 on CIFAR-10."""
    manual_seed(SEED)

    # Initialize wandb
    wandb.init(
        project="cifar10_vitsptlsa_ttv1_scratch",
        name=f"vitsptlsa_ttv1_bs{BATCH_SIZE}_e{N_EPOCHS}_lr{LEARNING_RATE}",
        config={
            "model": "ViT-SPT-LSA",
            "optimizer": "TikiTaka TTv1",
            "dataset": "CIFAR-10",
            "image_size": IMAGE_SIZE,
            "patch_size": PATCH_SIZE,
            "embed_dim": EMBED_DIM,
    ...
    print(f"  Our TTv1 result: {100 - best_accuracy:.1f}% error")
    print(f"{'='*60}")
