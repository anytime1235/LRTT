...
"""aihwkit example with LRTT: ResNet32 CNN with CIFAR-100 using LRTT layers.

CIFAR-100 dataset on a ResNet inspired network using LRTT (Low-Rank Tensor-Train)
...
RESULTS = os.path.join(os.getcwd(), "results", "RESNET_LRTT_CIFAR100")
os.makedirs(RESULTS, exist_ok=True)
WEIGHT_PATH = os.path.join(RESULTS, "cifar100_resnet_lrtt_model_weight.pth")
...
WARMUP_RATIO = 0.05  # Warmup ratio (10% of total epochs)
N_CLASSES = 100
...
TRANSFER_EVERY = 10000000  # Transfer A⊗B to C more frequently for better convergence
...
def load_pretrained_weights(analog_model):
    ...
    if len(analog_linear_layers) > 0 and len(pretrained_linear_layers) > 0:
        ...
        if analog_in_features == pretrained_fc.in_features:
            ...
            # Use Xavier initialization for CIFAR-100 classifier
...
def load_images():
    ...
    # Training transforms with standard augmentation for CIFAR-100
...
def main():
    """Train a PyTorch ResNet analog model with LRTT to classify CIFAR-100."""
    # Seed
    manual_seed(SEED)
    ...
    print("\nStarting LRTT training on CIFAR-100...")
    print("=" * 60)
