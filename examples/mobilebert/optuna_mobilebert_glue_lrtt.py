...
# than attention. Applied at BUILD time per bit-group (post-conversion overrides do NOT
# propagate to tiles). MobileBERT keys (substring of the module path):
#   query/key/value      -> attention.self.{query,key,value}
#   attention.output     -> attention.output.dense (O projection)
#   intermediate         -> layer-level intermediate.dense (main FFN1, pre-GELU)
#   output.dense         -> layer-level output.dense (main FFN2, pre-LayerNorm)
#   ffn                  -> the extra bottleneck FFNs (ffn.N.{intermediate,output}.dense)
# Example FFN 10-bit ADC: set 'intermediate'/'output.dense'/'ffn' adc=10.
LAYER_IO_BITS = {
    ...
    'intermediate':     {'dac': None, 'adc': None},   # main FFN1 (pre-GELU)
    'output.dense':     {'dac': None, 'adc': None},   # main FFN2 (pre-LayerNorm)
    'ffn':              {'dac': None, 'adc': None},    # extra bottleneck FFNs (ffn.N.*)
}
...
def get_study_name_suffix():
    ...
    # bit configs in separate studies. e.g. FFN 10-bit ADC -> _lio-f1a10-f2a10-fna10.
    if _any_layer_io_override():
        _abbr = {'query': 'q', 'key': 'k', 'value': 'v', 'attention.output': 'o',
                 'intermediate': 'f1', 'output.dense': 'f2', 'ffn': 'fn'}
        _parts = []
        for _k in ['query', 'key', 'value', 'attention.output',
...
def _module_io_key(name):
    """Map a MobileBERT Linear module name to its LAYER_IO_BITS key (None if unmatched).

    Disambiguation: 'ffn.N.*' (extra bottleneck FFNs) -> 'ffn' is checked BEFORE the
    layer-level 'intermediate'/'output.dense' so the bottleneck FFN sub-modules don't get
    mislabeled. attention.output is handled before the generic output.dense suffix.
    Bottleneck (input/attention/output.bottleneck) modules return None -> default bits.
    """
    ...
    if '.ffn.' in name:  # extra bottleneck FFNs: ffn.N.{intermediate,output}.dense
        return 'ffn'
    if 'intermediate.dense' in name:  # main FFN1 (ffn.* already handled above)
        return 'intermediate'
    if name.endswith('output.dense'):  # main FFN2 (attention.output/ffn handled above)
        return 'output.dense'
    return None
...
def create_frozen_analog_config(lrtt_config=None, out_noise=0.0, dac_bits=None, adc_bits=None):
    ...
    if lrtt_config is not None:
        ...
        # Apply (possibly FFN-specific) bits, overriding the inherited quantization.
...
def create_model(params):
    ...
    if any_frozen_analog:
        ...
        if not _any_layer_io_override():
            ...
            # Per-module IO bits for frozen-analog layers: group by resolved bits and
            # convert each group in its own pass (build-time bits). Frozen targets are the
            # linears NOT in frozen_exclude and NOT already-converted LRTT targets
            # (classifier/embedding_transformation map to default bits via
            # _module_io_key -> None). classifier and embedding_transformation are already
            # members of all_linear_names (returned by list_linear_layers).
...
def objective(trial, train_loader, eval_loader, tokenizer):
    ...
    # A/B dw_min: per-tile splittable.
    ...
    if SPLIT_AB_PARAMS.get('weight_scaling_omega', False):
        ...
        ab_weight_scaling_omega = a_weight_scaling_omega
    else:
...
def main():
    global TASK_NAME, NUM_LABELS, BATCH_SIZE, GRAD_ACCUM_STEPS, N_EPOCHS, WARMUP_STEPS, TRANSFER_METHOD, AB_DEVICE, A_DEVICE, B_DEVICE, C_DEVICE, IO_NOISE, FORWARD_INJECT, IS_PERFECT, NO_QUANT, LORA_TARGET, HEAD_LAYER, ENCODER_ANALOG, EMBEDDING_ANALOG, HEAD_ANALOG, BACKWARD_INP_BOUND, BACKWARD_OUT_BOUND
