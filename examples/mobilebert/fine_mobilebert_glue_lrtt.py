...
# than attention. Applied at build time per bit-group (see create_model): each group of
# modules sharing a resolved (dac, adc) is converted in its own pass. MobileBERT note:
# 'intermediate'/'output.dense' are the MAIN bottleneck FFN1/FFN2; 'ffn' covers the inner
# stacked ffn.{0,1,2}.{intermediate,output}.dense sub-blocks; 'attention.output' is the
# attention O projection (disambiguated from the FFN 'output.dense' key).
LAYER_IO_BITS = {
    ...
    'intermediate':     {'dac': None, 'adc': None},   # main bottleneck FFN1 (pre-activation)
    'output.dense':     {'dac': None, 'adc': None},   # main bottleneck FFN2 (pre-LayerNorm)
    'ffn':              {'dac': None, 'adc': None},    # stacked inner FFN sub-blocks (ffn.{0,1,2}.*)
}
...
HIST_RATE_STEPS = 500         # weight + signal histogram cadence (GLUE tasks vary widely)
...
def _module_io_key(name):
    """Map a MobileBERT encoder Linear module name to its LAYER_IO_BITS key (None if unmatched).

    Disambiguation order matters: the stacked inner FFN sub-blocks are named
    'ffn.{0,1,2}.intermediate.dense' / 'ffn.{0,1,2}.output.dense', so anything with '.ffn.'
    is grouped under the 'ffn' key BEFORE the generic 'intermediate'/'output.dense' keys.
    'attention.output.dense' (O projection) is matched before the FFN 'output.dense'.
    """
    ...
    # Inner stacked FFN blocks: ffn.0/1/2.{intermediate,output}.dense
    if '.ffn.' in name:
        return 'ffn'
    if name.endswith('intermediate.dense'):  # main bottleneck FFN1
        return 'intermediate'
    if name.endswith('output.dense'):  # main bottleneck FFN2 (attention/ffn already handled)
        return 'output.dense'
    return None
...
def create_frozen_analog_config(lrtt_config=None, out_noise=0.0, dac_bits=None, adc_bits=None):
    ...
    if lrtt_config is not None:
        ...
        # Fall back to module-level defaults when no per-module override is supplied
        # (dac_bits/adc_bits None → DAC_BITS/ADC_BITS), preserving prior behavior.
        _apply_quant_bits(
            rpu_config,
...
def create_model():
    ...
    if LORA_TARGET == "none":
        ...
        # No override → original single-pass conversion (byte-identical behavior).
    ...
    if any_frozen_analog:
        ...
        if not _any_layer_io_override():
            ...
            # classifier / embedding_transformation map to no key → fall back to defaults.
    ...
    if ENABLE_DIAGNOSTIC and _any_log:
        ...
        for sn, s in diag_state.items():
            ...
            make_diagnostic_plots(s["log"],
                os.path.join(RESULTS, f"{TASK_NAME}_diag_{sn}_{stamp}.png"),
                tile_label=label,
                A_ci=s["A_CI"], B_ci=s["B_CI"], C_ci=s["C_CI"])
            make_weight_dynamics_plots(s["log"],
                os.path.join(RESULTS, f"{TASK_NAME}_weight_dyn_{sn}_{stamp}.png"),
                tile_label=label)
            make_weight_hist_plots(s["log"],
                os.path.join(RESULTS, f"{TASK_NAME}_weight_hist_{sn}_{stamp}.png"),
                tile_label=label)
            make_xd_diagnostic_plots(s["log"],
                os.path.join(RESULTS, f"{TASK_NAME}_diag_xd_{sn}_{stamp}.png"),
                tile_label=label)
        ...
        for ep in range(1, diag_ep + 1):
            ...
            for sn in ("first", "last"):
                ...
                make_diagnostic_plots(slc,
                    os.path.join(RESULTS, f"{TASK_NAME}_diag_{sn}_{stamp}_ep{ep}.png"),
