...
# than attention. Applied at BUILD TIME by grouping target modules by resolved bits and
# converting each group in its own convert_to_analog pass (post-conversion override does
# NOT propagate to tiles). All-None = byte-identical to the original single-pass path.
...
def get_study_name_suffix():
    ...
    if _any_layer_io_override():
        _abbr = {'query': 'q', 'key': 'k', 'value': 'v', 'attention.output': 'o',
                 'bottleneck': 'bn', 'ffn.intermediate': 'ff1', 'ffn.output': 'ff2',
                 'intermediate': 'f1', 'output.dense': 'f2'}
        _parts = []
        for _k in ['query', 'key', 'value', 'attention.output', 'bottleneck',
                   'ffn.intermediate', 'ffn.output', 'intermediate', 'output.dense']:
...
def create_model(params):
    ...
    if any_frozen_analog:
        ...
        if not _any_layer_io_override():
            ...
            # embedding_transformation has no _module_io_key match -> falls back to defaults.
...
def objective(trial, train_loader, eval_features, eval_examples, tokenizer):
    ...
    # weight_scaling_omega: per-tile splittable via mapping_a / mapping_b override.
...
def main():
    global BATCH_SIZE, GRAD_ACCUM_STEPS, N_EPOCHS, WARMUP_STEPS, TRANSFER_METHOD, AB_DEVICE, A_DEVICE, B_DEVICE, C_DEVICE, IO_NOISE, FORWARD_INJECT, IS_PERFECT, NO_QUANT, LORA_TARGET, HEAD_LAYER, ENCODER_ANALOG, EMBEDDING_ANALOG, HEAD_ANALOG, BACKWARD_OUT_BOUND, _oom_retry_pending
