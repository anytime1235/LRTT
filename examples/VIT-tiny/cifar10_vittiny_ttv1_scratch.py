...
"""ViT-Tiny for CIFAR10 using TikiTaka TTv1 (TransferCompound) layers.
...
- Transformer blocks use TTv1 analog layers
"""
...
RESULTS = os.path.join(os.getcwd(), "results", "VITTINY_TTV1_SCRATCH")
os.makedirs(RESULTS, exist_ok=True)
WEIGHT_PATH = os.path.join(RESULTS, "cifar10_vittiny_ttv1_scratch_model_weight.pth")
...
class MultiHeadSelfAttention(nn.Module):
    """Multi-head self-attention with TTv1 analog layers."""
    def __init__(self, embed_dim, num_heads, dropout=0.0):
        ...
        self.qkv = AnalogLinear(embed_dim, embed_dim * 3, bias=True, rpu_config=create_ttv1_config())
        self.proj = AnalogLinear(embed_dim, embed_dim, bias=True, rpu_config=create_ttv1_config())
...
class MLP(nn.Module):
    """MLP block with TTv1 analog layers."""
    def __init__(self, in_features, hidden_features, out_features, dropout=0.0):
        super().__init__()
        self.fc1 = AnalogLinear(in_features, hidden_features, bias=True, rpu_config=create_ttv1_config())
        self.fc2 = AnalogLinear(hidden_features, out_features, bias=True, rpu_config=create_ttv1_config())
        self.act = nn.GELU()
...
class TransformerBlock(nn.Module):
    """Transformer block with TTv1 analog layers."""
...
class ViT_Tiny_TTv1(nn.Module):
    """Vision Transformer Tiny with TTv1 analog layers."""
...
def create_model():
    """Create ViT-Tiny model with TTv1 analog layers."""
    model = ViT_Tiny_TTv1(
    ...
    print(f"\nCreated ViT-Tiny (TTv1): {count_parameters(model):,} params, {count_analog_layers(model)} analog layers")
    print(f"  TTv1 config: transfer_every={TRANSFER_EVERY}, devices: A={DEVICE_A}, C={DEVICE_C}")
    return model
...
def main():
    """Train ViT-Tiny with TTv1 on CIFAR-10."""
    manual_seed(SEED)
    if USE_CUDA:
        torch.cuda.manual_seed(SEED)

    wandb.init(
        project="cifar10_vittiny_ttv1_scratch",
        name=f"vittiny_ttv1_te{TRANSFER_EVERY}_bs{BATCH_SIZE}",
        config={
            "model": "ViT-Tiny-TTv1", "dataset": "CIFAR-10",
            "embed_dim": EMBED_DIM, "depth": DEPTH, "num_heads": NUM_HEADS,
            "transfer_every": TRANSFER_EVERY, "device_a": DEVICE_A, "device_c": DEVICE_C,
