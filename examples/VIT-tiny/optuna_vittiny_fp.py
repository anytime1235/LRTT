# -*- coding: utf-8 -*-
"""Optuna hyperparameter sweep for ViT-Tiny with Digital FP on CIFAR-10.

Usage:
    # Run trials (results are automatically saved and can be resumed)
    python optuna_vittiny_fp.py --n-trials 50

    # Resume existing study and add more trials
    python optuna_vittiny_fp.py --n-trials 30  # continues from saved results

    # Parallel execution (run in multiple terminals, same study name)
    python optuna_vittiny_fp.py --n-trials 20 &
    python optuna_vittiny_fp.py --n-trials 20 &
    python optuna_vittiny_fp.py --n-trials 20 &

    # Visualize results without running new trials
    python optuna_vittiny_fp.py --visualize

    # Use a different study name (for separate experiments)
    python optuna_vittiny_fp.py --n-trials 50 --study-name vittiny_fp_frozen

    # Real-time dashboard (install: pip install optuna-dashboard)
    optuna-dashboard sqlite:///results/optuna_vittiny_fp/optuna_vittiny_fp_main.db

    # Reset study (delete DB to start fresh)
    rm results/optuna_vittiny_fp/optuna_vittiny_fp_main.db

Results are stored in:
    - SQLite DB: results/optuna_vittiny_fp/optuna_<study_name>.db
    - JSON summary: results/optuna_vittiny_fp/best_params_*.json
    - Visualization: results/optuna_vittiny_fp/visualization_*.png
"""
...
DEFAULT_STUDY_NAME = "vittiny_fp_main"
...
RESULTS = os.path.join(os.getcwd(), "results", "optuna_vittiny_fp")
