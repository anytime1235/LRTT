# -*- coding: utf-8 -*-
"""Optuna hyperparameter sweep for ViT-Tiny with TTv1 on CIFAR-10.

Usage:
    python optuna_vittiny_ttv1.py --n-trials 50
    python optuna_vittiny_ttv1.py --visualize
"""
...
DEFAULT_STUDY_NAME = "vittiny_ttv1_main"

USE_CUDA = torch.cuda.is_available()
DEVICE = device("cuda" if USE_CUDA else "cpu")

PATH_DATASET = os.path.join(os.getcwd(), "data", "DATASET")
RESULTS = os.path.join(os.getcwd(), "results", "optuna_vittiny_ttv1")
...
def create_ttv1_config(transfer_every):
...
def objective(trial):
    ...
    print(f"{'='*70}")
    print(f"  transfer_every={transfer_every}, optimizer={optimizer_name}")
    ...
    model = None
    try:
        rpu_config = create_ttv1_config(transfer_every)
        model = ViT_Tiny_TTv1(
...
def main():
    parser = argparse.ArgumentParser(description="Optuna sweep for ViT-Tiny TTv1")
    ...
    study_name = args.study_name or f"vittiny_ttv1_bs{BATCH_SIZE}_{get_study_name_suffix()}"
