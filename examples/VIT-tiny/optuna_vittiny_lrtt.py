# -*- coding: utf-8 -*-
"""Optuna hyperparameter sweep for ViT-Tiny with LRTT on CIFAR-10.

Usage:
    python optuna_vittiny_lrtt.py --n-trials 50
    python optuna_vittiny_lrtt.py --visualize
"""
...
DEFAULT_STUDY_NAME = "vittiny_lrtt_main"

USE_CUDA = torch.cuda.is_available()
DEVICE = device("cuda" if USE_CUDA else "cpu")

PATH_DATASET = os.path.join(os.getcwd(), "data", "DATASET")
RESULTS = os.path.join(os.getcwd(), "results", "optuna_vittiny_lrtt")
...
def main():
    parser = argparse.ArgumentParser(description="Optuna sweep for ViT-Tiny LRTT")
    ...
    study_name = args.study_name or f"vittiny_lrtt_bs{BATCH_SIZE}_{get_study_name_suffix()}"
    ...
    study = optuna.create_study(
        ...
        sampler=ConfigAwareBoTorchSampler(consider_running_trials=True), pruner=optuna.pruners.NopPruner(),
        load_if_exists=True,
    )
