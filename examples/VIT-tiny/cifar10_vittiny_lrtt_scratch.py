...
"""ViT-Tiny for CIFAR10 using LRTT (Low-Rank TikiTaka Training) layers.
...
- Transformer blocks use LRTT analog layers
"""
...
RESULTS = os.path.join(os.getcwd(), "results", "VITTINY_LRTT_SCRATCH")
os.makedirs(RESULTS, exist_ok=True)
WEIGHT_PATH = os.path.join(RESULTS, "cifar10_vittiny_lrtt_scratch_model_weight.pth")
...
class PatchEmbedding(nn.Module):
    """Standard patch embedding (Conv2d projection) - Digital."""
...
class MultiHeadSelfAttention(nn.Module):
    """Multi-head self-attention with LRTT analog layers."""
    def __init__(self, embed_dim, num_heads, dropout=0.0):
        ...
        self.qkv = AnalogLinear(embed_dim, embed_dim * 3, bias=True, rpu_config=create_lrtt_config())
        self.proj = AnalogLinear(embed_dim, embed_dim, bias=True, rpu_config=create_lrtt_config())
...
class MLP(nn.Module):
    """MLP block with LRTT analog layers."""
    def __init__(self, in_features, hidden_features, out_features, dropout=0.0):
        super().__init__()
        self.fc1 = AnalogLinear(in_features, hidden_features, bias=True, rpu_config=create_lrtt_config())
        self.fc2 = AnalogLinear(hidden_features, out_features, bias=True, rpu_config=create_lrtt_config())
        self.act = nn.GELU()
...
class TransformerBlock(nn.Module):
    """Transformer block with LRTT analog layers."""
...
class ViT_Tiny_LRTT(nn.Module):
    """Vision Transformer Tiny with LRTT analog layers."""
    def __init__(self, image_size=32, patch_size=4, in_channels=3, num_classes=10,
        ...
        # LRTT analog transformer blocks
...
def create_model():
    """Create ViT-Tiny model with LRTT analog layers."""
    model = ViT_Tiny_LRTT(
    ...
    print(f"\nCreated ViT-Tiny model (LRTT):")
    print(f"  Embed dim: {EMBED_DIM}, Depth: {DEPTH}, Heads: {NUM_HEADS}")
    print(f"  Trainable parameters: {num_params:,}")
    print(f"  Analog (LRTT) layers: {num_analog}")
    print(f"  LRTT config: rank={LRTT_RANK}, transfer_every={TRANSFER_EVERY}, lora_alpha={LORA_ALPHA}")
    print(f"  Devices: AB={AB_DEVICE}, C={C_DEVICE}")
...
def main():
    """Train ViT-Tiny with LRTT on CIFAR-10."""
    manual_seed(SEED)
    if USE_CUDA:
        torch.cuda.manual_seed(SEED)

    wandb.init(
        project="cifar10_vittiny_lrtt_scratch",
        name=f"vittiny_lrtt_r{LRTT_RANK}_te{TRANSFER_EVERY}_bs{BATCH_SIZE}",
        config={
            "model": "ViT-Tiny-LRTT", "dataset": "CIFAR-10",
            "embed_dim": EMBED_DIM, "depth": DEPTH, "num_heads": NUM_HEADS,
            "lrtt_rank": LRTT_RANK, "transfer_every": TRANSFER_EVERY, "lora_alpha": LORA_ALPHA,
            "reinit_mode": REINIT_MODE, "ab_device": AB_DEVICE, "c_device": C_DEVICE,
