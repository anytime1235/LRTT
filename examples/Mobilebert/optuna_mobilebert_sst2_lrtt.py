...
Usage:
    ...
    python optuna_mobilebert_sst2_lrtt.py --n-trials 50 --optimizer AnalogSGD --reinit-mode hybrid --no-wd --no-momentum --no-nesterov --batch-size 64 --epochs 15 --warmup-steps 500 --transfer-method set --no-io-noise --lora-target qkv
...
def objective(trial, train_loader, eval_loader, tokenizer):
    ...
    if OPT_CONFIG['no_transfer']:
        ...
        rank = 2 ** rank_exp
        fast_lr = trial.suggest_float('fast_lr', 6e-5, 2e1, log=True)
...
def main():
    global BATCH_SIZE, GRAD_ACCUM_STEPS, N_EPOCHS, WARMUP_STEPS, TRANSFER_METHOD, AB_DEVICE, C_DEVICE, IO_NOISE, LORA_TARGET, HEAD_LAYER, ENCODER_ANALOG, EMBEDDING_ANALOG, HEAD_ANALOG, BACKWARD_OUT_BOUND, _oom_retry_pending
