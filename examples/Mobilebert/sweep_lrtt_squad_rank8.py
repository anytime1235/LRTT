...
# Default LRTT config (rank=8 best from sweep_20260206_155404, F1=70.55)
DEFAULT_LRTT_PARAMS = {
    "learning_rate": 0.01421,
    "transfer_lr": 0.000380,
    "transfer_every": 100,
}
...
MIN_LR_RATE_VALUES = [0]  # Original sweep used 0 (standard linear decay)
N_TRIALS = 1
NUM_EPOCHS = 3
...
def create_lrtt_config(
    ...
    """Create LRTT config matching original sweep_lrtt_all_tasks.py (2/6 version)."""
    import math
...
def run_trial(trial: optuna.Trial, task_name: str, train_loader, eval_loader,
    ...
    if MODE == "sixt1c_lora":
        ...
        # lrtt mode: fix best params, sweep min_lr_rate only
        params = {
            "learning_rate": DEFAULT_LRTT_PARAMS["learning_rate"],
            "transfer_lr": DEFAULT_LRTT_PARAMS["transfer_lr"],
            "transfer_every": DEFAULT_LRTT_PARAMS["transfer_every"],
            "min_lr_rate": trial.suggest_categorical("min_lr_rate", MIN_LR_RATE_VALUES),
    ...
    try:
        ...
        # Create linear scheduler with min LR floor
        num_training_steps = len(train_loader) * NUM_EPOCHS
        scheduler = get_linear_schedule_with_min_lr(
            optimizer,
            num_warmup_steps=WARMUP_STEPS,
            num_training_steps=num_training_steps,
            min_lr_rate=params.get("min_lr_rate", 0.0),
        )
...
def run_task_sweep(task_name: str, device: torch.device, results_dir: str, n_trials: int = 10, n_jobs: int = 1):
    ...
    # Enqueue trials: one per min_lr_rate value (other params fixed at best)
    if MODE == "sixt1c_lora":
        study.enqueue_trial(DEFAULT_SIXT1C_LORA_PARAMS)
    else:
        for mlr in MIN_LR_RATE_VALUES:
            study.enqueue_trial({"min_lr_rate": mlr})
    ...
    all_trials_result = {
        ...
        "search_space": {
            ...
            "min_lr_rate": {"values": MIN_LR_RATE_VALUES, "scale": "categorical"},
        },
        "fixed_params": {
            "rank": RANK,
            "lora_alpha": LORA_ALPHA,
...
def main():
    ...
    if MODE == "sixt1c_lora":
        ...
        print(f"  min_lr_rate: {MIN_LR_RATE_VALUES} (categorical)")
        print()
