...
Usage:
    python optuna_mobilebert_sst2_lrtt_embtranstrain.py --n-trials 50
    python optuna_mobilebert_sst2_lrtt_embtranstrain.py --visualize
    python optuna_mobilebert_sst2_lrtt_embtranstrain.py --n-trials 50 --optimizer AnalogSGD --reinit-mode hybrid --no-wd --no-momentum --no-nesterov --batch-size 64 --epochs 3 --warmup-steps 189 --transfer-method set --no-io-noise --lora-target qkv

All flags:
    python optuna_mobilebert_sst2_lrtt_embtranstrain.py \
        ...
        --warmup-steps <int>        # LR warmup steps (default: 189, ~6% of 3 epochs)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Optuna sweep for MobileBERT SST-2 LRTT (emb_trans trainable)")
