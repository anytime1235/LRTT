...
# coding=utf-8
"""LRTT Bayesian Optimization for SQuAD - RANK 8, Value-only, DECAY reinit (15 epoch).

15-epoch sweep with cosine+minLR scheduler and early stopping.
reinit_mode=decay, lifetime added to search space.
Search space narrowed around Adam 3-epoch best: lr=0.0396, t_lr=3.12e-4, te=193.
"""
...
sys.path.insert(0, '/data/LRTT_vit/src')
...
LR_MIN, LR_MAX = 1e-4, 1e-2                             # learning rate (log)
TRANSFER_LR_MIN, TRANSFER_LR_MAX = 1e-12, 1e-2          # transfer learning rate (log)
TRANSFER_EVERY_MIN, TRANSFER_EVERY_MAX = 100, 10000000000     # transfer frequency (int, log)
LIFETIME_MIN, LIFETIME_MAX = 10, 100000                  # lifetime (log)
...
# Default LRTT config (seeded from Adam 3-epoch V-only best)
DEFAULT_LRTT_PARAMS = {
    "learning_rate": 0.009676045941584346,
    "transfer_lr": 7.640386441380507e-05,
...
WANDB_PROJECT = "lrtt-squad-rank8-v-only-15ep-decay-sweep"
...
def create_lrtt_config(
    ...
    """Create LRTT config with decay reinit and variable lifetime.
    ...
    Args:
        ...
        lifetime: Retention lifetime for A/B tiles
...
def create_glue_model(task_name: str, params: Dict, device: torch.device) -> nn.Module:
    """Create GLUE model with LRTT (decay reinit)."""
...
def create_squad_model(params: Dict, device: torch.device) -> nn.Module:
    """Create SQuAD model with LRTT (decay reinit)."""
...
def run_trial(trial: optuna.Trial, task_name: str, train_loader, eval_loader,
    ...
    params = {
        ...
        "lifetime": trial.suggest_int("lifetime", LIFETIME_MIN, LIFETIME_MAX, log=True),
    }
...
def run_task_sweep(task_name: str, device: torch.device, results_dir: str, n_trials: int = 10, n_jobs: int = 1):
    ...
    all_trials_result = {
        ...
        "search_space": {
            ...
            "lifetime": {"min": LIFETIME_MIN, "max": LIFETIME_MAX, "scale": "int_log"},
        },
        "fixed_params": {
            "rank": RANK,
            "lora_alpha": LORA_ALPHA,
    ...
    all_trials_file = os.path.join(results_dir, f"{task_name}_rank{RANK}_v_only_15ep_decay_all_trials.json")
...
def main():
    ...
    print("="*60)
    print("LRTT Bayesian Optimization - SQuAD RANK 8 (Value-only, 15ep, DECAY)")
    print("="*60)
    ...
    print(f"  lifetime: [{LIFETIME_MIN}, {LIFETIME_MAX}] (int, log)")
    print()
