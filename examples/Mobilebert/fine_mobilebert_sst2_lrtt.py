...
Based on fine_mobilebert_squad_lrtt.py, adapted for SST-2 classification.
...
WEIGHT_PATH = os.path.join(RESULTS, "fine_mobilebert_sst2_lrtt_model_weight.pth")
...
def main():
    """Train MobileBERT with LRTT on SST-2."""
    manual_seed(SEED)
    set_seed(SEED)
    if USE_CUDA:
    ...
    wandb.log({"epoch": 0, "eval/accuracy": init_acc, "eval/val_loss": init_val_loss})
    print(f"Initial eval: Accuracy={init_acc:.2f}% | Val loss={init_val_loss:.4f}")
    ...
    for epoch in tqdm(range(1, N_EPOCHS + 1), desc="Training"):
        ...
        wandb.log({
            ...
            "eval/accuracy": eval_acc, "eval/val_loss": val_loss,
    ...
    if ENABLE_DIAGNOSTIC and first_log:
        ...
        with open(json_path, 'w') as f:
            json.dump({
                ...
                "best_accuracy": best_acc, "best_epoch": best_epoch,
