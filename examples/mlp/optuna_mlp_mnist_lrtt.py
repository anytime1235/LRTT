...
# Set ADC(out_res)/DAC(inp_res) bits per analog Linear module — e.g. give linear1 a
# higher/lower precision than the swept default. Applied at build time (each bit-group
# converted in its own pass) so each matched tile's forward+backward inp_res(DAC)/
# out_res(ADC) reflect the override. Keys are this MLP's analog Linear module names.
# NOTE: linear1 is the only LRTT-eligible / analog-eligible layer in this 2-layer MLP
# (the classifier head stays digital), so this feature is effectively a single-layer
# knob here; it is kept structurally consistent with the BERT/transformer scripts.
LAYER_IO_BITS = {
    'linear1': {'dac': None, 'adc': None},  # input projection (the sole analog Linear)
}
...
def get_study_name_suffix():
    ...
    # bit configs in separate studies. e.g. linear1 10-bit ADC -> _lio-l1a10.
...
def _module_io_key(name):
    """Map an analog Linear module name to its LAYER_IO_BITS key (None if unmatched).

    This MLP only has a single analog-eligible Linear layer (linear1); the classifier
    output head stays digital and is never matched here.
    """
    if 'linear1' in name:
        return 'linear1'
    return None
...
def create_frozen_analog_config(lrtt_config=None, out_noise=0.0, dac_bits=None, adc_bits=None):
    ...
    if lrtt_config is not None:
        ...
        # Apply (possibly per-layer) bits, overriding the inherited LRTT quantization.
