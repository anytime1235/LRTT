...
# inp_res(DAC)/out_res(ADC). Keys are ALBERT structural module names (the layer index
# is shared across the model). Example FFN 10-bit ADC: set 'ffn'/'ffn_output' adc=10.
LAYER_IO_BITS = {
...
def _module_io_key(name):
    ...
    if name.endswith('ffn_output'):  # FFN2 (must precede the 'ffn' substring check)
        return 'ffn_output'
...
def main():
    global BATCH_SIZE, GRAD_ACCUM_STEPS, N_EPOCHS, WARMUP_STEPS, TRANSFER_METHOD, AB_DEVICE, A_DEVICE, B_DEVICE, C_DEVICE, IO_NOISE, FORWARD_INJECT, IS_PERFECT, NO_QUANT, LORA_TARGET, HEAD_LAYER, ENCODER_ANALOG, EMBEDDING_ANALOG, HEAD_ANALOG, BACKWARD_OUT_BOUND, RESULTS, _oom_retry_pending
