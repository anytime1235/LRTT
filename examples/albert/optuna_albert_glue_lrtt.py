...
# than attention. Applied at build time (each bit-group converted in its own pass) to each
# matched tile's forward+backward inp_res(DAC)/out_res(ADC). Keys match ALBERT module names:
# query/key/value (attention Q/K/V), attention.dense (attention output O), ffn (FFN1),
# ffn_output (FFN2). Example FFN 10-bit ADC: set 'ffn'/'ffn_output' adc=10.
LAYER_IO_BITS = {
...
def _module_io_key(name):
    ...
    ALBERT module suffixes: attention.{query,key,value,dense}, ffn, ffn_output. Order
    matters: 'ffn_output' must be checked before 'ffn' (the latter is a substring of the
    former), and 'attention.dense' (FFN-vs-attention-output disambiguation) is explicit.
    """
    ...
    if name.endswith('ffn_output'):  # FFN2 (check before 'ffn' — substring overlap)
        return 'ffn_output'
...
def create_model(params):
    ...
    if any_frozen_analog:
        ...
        if not _any_layer_io_override():
            ...
            # embedding_hidden_mapping_in has no LAYER_IO_BITS key -> _resolve_io_bits
            # returns the swept defaults (unaffected by overrides), matching the
            # single-pass behavior for that module.
