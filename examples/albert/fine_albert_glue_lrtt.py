# -*- coding: utf-8 -*-
"""ALBERT + GLUE with LRTT (Low-Rank TikiTaka Training).

Single-run training script for ALBERT on GLUE tasks using LRTT analog layers.
...
Usage:
    python fine_albert_glue_lrtt.py --task sst2
    python fine_albert_glue_lrtt.py --task mrpc
    python fine_albert_glue_lrtt.py --task stsb

Based on fine_albert_sst2_lrtt.py, generalized for all GLUE tasks.
...
_parser = argparse.ArgumentParser(description="ALBERT GLUE LRTT fine-tuning")
_parser.add_argument('--task', type=str, default='sst2',
                     choices=list(TASK_TO_KEYS.keys()),
                     help='GLUE task name (default: sst2)')
_parser.add_argument('--grad-accum-steps', type=int, default=None,
                     help='Gradient accumulation steps (overrides GRAD_ACCUM_STEPS)')
_args, _ = _parser.parse_known_args()
TASK_NAME = _args.task
if _args.grad_accum_steps is not None:
...
# Paths
RESULTS = os.path.join(os.getcwd(), "results", "ALBERT_GLUE_LRTT_FINE")
os.makedirs(RESULTS, exist_ok=True)
WEIGHT_PATH = os.path.join(RESULTS, f"fine_albert_glue_lrtt_{TASK_NAME}_model_weight.pth")
...
# propagate to tiles), to both the LRTT pass and the frozen-analog pass. Keys match
# ALBERT encoder module-name substrings (see _module_io_key). Example FFN 10-bit ADC:
...
# WandB
WANDB_PROJECT = f"albert-{TASK_NAME}-lrtt-fine"
...
def _module_io_key(name):
    ...
    ALBERT encoder modules: attention.{query,key,value,dense}, ffn, ffn_output.
    Order matters: 'ffn_output' must be matched before 'ffn' (substring), and
    'attention.dense' is distinct from the FFN outputs.
    """
    ...
    if name.endswith('ffn_output'):  # FFN2 (pre-LayerNorm)
        return 'ffn_output'
    if name.endswith('ffn'):  # FFN1 (pre-GELU); checked after ffn_output
        return 'ffn'
    return None
...
def create_frozen_analog_config(lrtt_config=None, out_noise=0.0, dac_bits=None, adc_bits=None):
    ...
    if lrtt_config is not None:
        ...
        # No-op when dac_bits/adc_bits are None (default path uses inherited values).
        ...
        # Per-module overrides (dac_bits/adc_bits) take precedence; else module defaults.
        _apply_quant_bits(
            rpu_config,
...
def create_model():
    ...
    if LORA_TARGET == "none":
        ...
        # All-None fast-path is byte-identical to the original single-pass conversion.
    ...
    if any_frozen_analog:
        ...
        # Per-module IO bits: with LAYER_IO_BITS overrides, group frozen-analog targets by
        # resolved bits and convert each group separately. All-None fast-path is identical
        # to the original single-pass conversion.
        if not _any_layer_io_override():
...
def main():
    """Train ALBERT with LRTT on GLUE."""
    metric_name = TASK_TO_METRIC[TASK_NAME]
    ...
    wandb.init(
        ...
        name=f"albert_lrtt_{TASK_NAME}_r{LRTT_RANK}_te{TRANSFER_EVERY}_bs{BATCH_SIZE}",
        config={
            ...
            "epochs": N_EPOCHS, "batch_size": BATCH_SIZE, "grad_accum_steps": GRAD_ACCUM_STEPS,
    ...
    wandb.log({"epoch": 0, f"eval/{metric_name}": init_acc, "eval/val_loss": init_val_loss})
    print(f"Initial eval: {metric_name}={init_acc:.2f}% | val_loss={init_val_loss:.4f}")
