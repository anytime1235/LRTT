...
def create_analog_config():
    ...
    class SpatialLRTTSimulatorTileFullAnalog(SpatialLRTTSimulatorTile):
        """Spatial LRTT tile for fullanalog training (C-only update)."""
    ...
    @dataclass
    class SpatialPythonLRTTDeviceFullAnalog(SpatialPythonLRTTDevice):
        """Spatial LRTT device for fullanalog training."""

        def get_default_tile_module_class(self):
            """Return the fullanalog spatial LRTT tile class."""
            return SpatialLRTTSimulatorTileFullAnalog

    # CRITICAL: Custom RPU Config to override tile_class
    @dataclass
    class PythonLRTTRPUConfigFullAnalog(PythonLRTTRPUConfig):
        """Custom RPU Config that uses SpatialLRTTSimulatorTileFullAnalog."""

        tile_class: type = SpatialLRTTSimulatorTileFullAnalog

        def get_default_tile_module_class(self, out_size: int = 0, in_size: int = 0) -> type:
            """Return the fullanalog tile class."""
            return SpatialLRTTSimulatorTileFullAnalog

    # Use Spatial LRTT FullAnalog device which updates only C matrix
    device_config = SpatialPythonLRTTDeviceFullAnalog(
