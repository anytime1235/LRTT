...
"""aihwkit example 18 with LRTT: ResNet32 CNN with CIFAR100 using LRTT layers.

CIFAR100 dataset on a ResNet inspired network using LRTT (Low-Rank Tensor-Train)
...
RESULTS = os.path.join(os.getcwd(), "results", "RESNET_REGULAR_LRTT_SCRATCH_CIFAR100")
...
BASELINE_CHECKPOINT_PATH = "results/RESNET_FULLANALOG_SCRATCH/fullanalog_scratch_model_weight_10epoch.pth"  # Example: "results/RESNET_FULLANALOG_SCRATCH/fullanalog_scratch_model_weight_0epoch.pth"
LOAD_BASELINE = True  # Set to True to load baseline weights

# Training parameters
SEED = 1
N_EPOCHS = 1100  # Reduced for LRTT demonstration
...
RELORA_RESET_EVERY = 500  # LR cycle period for jagged cosine (≈10 epochs with batch_size=128)
                            # CIFAR-100: 50000/128 ≈ 390 steps/epoch, so 10 epochs = 3900 steps
...
def main():
    """Train a PyTorch ResNet analog model with LRTT to classify CIFAR100."""
    # Seed
    manual_seed(SEED)
    ...
    print("\nStarting LRTT training on CIFAR100...")
    if ENABLE_RELORA:
