#!/usr/bin/env python3
"""constantstepideal multi-seed (V2) — using ideal-optimal hyperparams (T267, abml=10).

Source defaults already T267 (Trial 267, F1=84.98). We only patch:
- N_EPOCHS=5, SEED=42/43/44/45, ENABLE_DIAGNOSTIC=True
- minimal DIAG_GROUPS (G coherence + basic norms)
- Leave all hyperparams at source defaults (T267).
"""
...
OUT_DIR = REPO / "examples/bert/results/plots/diag_ideal_v2_t267"
RUN_STAMP = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")

VARIANTS = [
    dict(tag=f"ideal_v2_t267_seed{seed}", gpu=gpu, seed=seed)
...
def main():
    ...
    procs = []
    for v in VARIANTS:
        unique = f"ideal_v2_{RUN_STAMP}_{v['tag']}"
        src = base
        # Only patch experiment-level settings + seed; leave hyperparams at source T267 defaults
        ...
        src = patch(src, "LORA_TARGET", '"qkvo"')  # explicit
        ...
        print(f"  [GPU {v['gpu']}] launched: {v['tag']} (seed={v['seed']}, T267 ideal-optimal)  pid={proc.pid}",
              flush=True)
