...
def objective(trial, train_loader, eval_features, eval_examples, tokenizer):
    ...
    # Hyperparameters
    learning_rate = trial.suggest_float('learning_rate', 8e-4, 7e-3, log=True)
    ...
    if OPT_CONFIG['no_transfer']:
        ...
        transfer_lr = trial.suggest_float('transfer_lr', 8e-2, 3e3, log=True)
        transfer_every = trial.suggest_int('transfer_every', 1, 400, log=True)
        rank_exp = trial.suggest_int('rank_exp', 0, 6)
        rank = 2 ** rank_exp
        fast_lr = trial.suggest_float('fast_lr', 1e-2, 4e0, log=True)
        tau_sec = trial.suggest_float('tau_sec', 1e1, 1e7, log=True)  # 0 = no decay
...
def main():
    global BATCH_SIZE, GRAD_ACCUM_STEPS, N_EPOCHS, WARMUP_STEPS, TRANSFER_METHOD, AB_DEVICE, C_DEVICE, IO_NOISE, FORWARD_INJECT, IS_PERFECT, NO_QUANT, LORA_TARGET, HEAD_LAYER, ENCODER_ANALOG, HEAD_ANALOG, BACKWARD_OUT_BOUND, _oom_retry_pending
