# -*- coding: utf-8 -*-
"""BERT + SQuAD with LRTT (Low-Rank TikiTaka Training).

Single-run training script for BERT on SQuAD using LRTT analog layers.
...
# Paths
RESULTS = os.path.join(os.getcwd(), "results", "BERT_SQUAD_LRTT_FINE")
os.makedirs(RESULTS, exist_ok=True)
WEIGHT_PATH = os.path.join(RESULTS, "fine_bert_squad_lrtt_model_weight.pth")
...
WEIGHT_DECAY = 0.0
EARLY_STOP_PATIENCE = 5  # raised so 5-epoch analysis runs never trigger early stop
TRAIN_LOSS_EARLY_STOP_PATIENCE = 5  # raised so 5-epoch analysis runs never trigger early stop
...
# Per-module IO bit override (MANUALLY EDIT; None = use the module-default DAC_BITS/ADC_BITS).
# Set ADC(out_res)/DAC(inp_res) bits per module TYPE — e.g. give FFN higher precision
# than attention. Applied at BUILD time per bit-group (post-conversion override does NOT
# propagate to tiles). Example FFN 10-bit ADC: set 'intermediate'/'output.dense' adc=10.
# When all entries are None, conversion uses the original single-pass path (no regression).
LAYER_IO_BITS = {
...
LORA_TARGET_MODULES = {
    ...
    "allnobn": None,  # Same as all for BERT (no bottleneck) (72 layers)
...
# Per-layer tile selection. Reuses LORA_TARGET_MODULES vocabulary.
#
# Shorthand string:
#   "first_last"  → first & last LRTT-converted layer × LORA_TARGET modules (Phase 2 default)
#   "all"         → every LRTT-converted layer × LORA_TARGET modules
#
# Explicit dict {layer_idx: module_selector}, module_selector being:
#   "match"             — inherit LORA_TARGET (= this layer's LRTT-converted modules)
#   key from LORA_TARGET_MODULES — "qkvo"|"qkv"|"qonly"|"konly"|"vonly"|"ffn"|"dense"|"all"
#   list of module names — e.g. ["query", "key"]
#
# Examples:
#   {0: "qkvo", 11: "qkvo"}                       # layer 0,11 × {q,k,v,o}
#   {0: "match", 6: "match", 11: "match"}         # layers 0,6,11 (each = LORA_TARGET)
#   {0: "qonly", 11: "qkvo"}                      # heterogeneous per layer
#   {0: ["query","key"], 11: "match"}             # explicit module names
#
# Intersection rule: actual monitored set = LRTT-converted ∩ selected layers ∩ selected modules.
# Empty intersection → warning + skip.
DIAG_TILES = "first_last"

# Rate limits (steps; <=0 means every step)
ERANK_RATE_LIMIT_STEPS = 0    # erank/SVD cost gate
HIST_RATE_STEPS = 1844        # weight + signal histogram cadence (= 1 epoch @ effective bs=48)
...
def create_frozen_analog_config(lrtt_config=None, out_noise=0.0, dac_bits=None, adc_bits=None):
    ...
    if lrtt_config is not None:
        ...
        # Apply (possibly FFN-specific) bits, overriding the inherited LRTT quantization.
        # No-op when dac_bits/adc_bits are None (default path): byte-identical to before.
...
def get_lrtt_target_module_names(lora_target):
    ...
    if lora_target == "none":
        ...
        return None  # Same as all for BERT (no bottleneck) (72 layers)
...
def create_model():
    ...
    LoRA Target Options (LORA_TARGET):
        ...
        - ffn: projection + FFN layers → LRTT Analog (24 layers)
...
def _short_tile_name(full_name):
    """Convert e.g. 'bert.encoder.layer.0.attention.self.query.analog_module' → 'L0.attention.self.query'."""
    import re
...
def resolve_diag_tiles(model, diag_tiles_cfg, lora_target_key):
    ...
    diag_tiles_cfg :
        "first_last"           — first & last LRTT-converted tile (Phase 2 default)
...
# -----------------------------------------------------------------------------
# Per-group diagnostic helpers (Step 4a — defined; consumers wire up in Step 4b)
# -----------------------------------------------------------------------------
...
def make_diagnostic_plots(log_data, output_path, tile_label="",
    ...
    # sigma_1 overlay (twin axis) — collapse signal
...
def make_weight_dynamics_plots(log_data, output_path, tile_label=""):
    ...
    # (1,1) Delta C_eff (separate panel — transfer-driven, different scale)
    ax = axes[1, 1]
...
def main():
    ...
    wandb.init(
        ...
        name=f"bert_lrtt_r{LRTT_RANK}_te{TRANSFER_EVERY}_bs{BATCH_SIZE}",
        config={
    ...
    if ENABLE_DIAGNOSTIC:
        ...
        # Install hooks on every tile in diag_state (Step 6b.3).
