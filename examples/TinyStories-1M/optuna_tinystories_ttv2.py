# -*- coding: utf-8 -*-
"""Optuna hyperparameter sweep for TinyStories-1M with TTv2 (ChoppedTransferCompound + fast_lr).

TTv2 uses ChoppedTransferCompound with fast_lr for improved training dynamics.

Usage:
    # SGD with all hyperparams tuned
    python optuna_tinystories_ttv2.py --n-trials 50 --optimizer sgd

    # Adam with weight decay tuning
    python optuna_tinystories_ttv2.py --n-trials 50 --optimizer adam

    # SGD without momentum tuning
    python optuna_tinystories_ttv2.py --n-trials 50 --optimizer sgd --no-momentum

    # Visualize results
    python optuna_tinystories_ttv2.py --visualize --optimizer sgd

    # Dashboard
    optuna-dashboard sqlite:///results/optuna_tinystories_ttv2/optuna_tinystories_ttv2_sgd.db
"""
...
RESULTS = os.path.join(os.getcwd(), "results", "optuna_tinystories_ttv2")
...
def objective(trial):
    ...
    # auto_granularity does NOT need scaling
    _current_config = {
    ...
    print(f"\n{'='*70}")
    print(f"Trial {trial.number} Starting (TTv2)")
    print(f"{'='*70}")
    print(f"  transfer_every={transfer_every} (actual={transfer_every*grad_accum_steps}), fast_lr={fast_lr:.4f}")
...
def visualize_study(study, save_dir):
    ...
    ax = axes[1, 1]
    fast_lrs = [t.params.get('fast_lr', 0.1) for t in complete_trials]
    ax.scatter(fast_lrs, perplexities, alpha=0.6)
    ax.set_xscale('log')
    ax.set_xlabel('Fast LR')
    ax.set_ylabel('Perplexity')
    ax.set_title('Fast LR vs Perplexity')
    ax.grid(True, alpha=0.3)

    ax = axes[1, 2]
    in_chop_probs = [t.params.get('in_chop_prob', 0.0) for t in complete_trials]
    ax.scatter(in_chop_probs, perplexities, alpha=0.6)
    ax.set_xlabel('In Chop Prob')
    ax.set_ylabel('Perplexity')
    ax.set_title('In Chop Prob vs Perplexity')
    ax.grid(True, alpha=0.3)
...
def print_study_summary(study):
    ...
    print("\n" + "=" * 60)
    print("STUDY SUMMARY (TTv2 - ChoppedTransferCompound)")
    print("=" * 60)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Optuna sweep for TinyStories-1M TTv2")
    ...
    study_name = args.study_name or f"tinystories_ttv2_bs{BATCH_SIZE}_{get_study_name_suffix()}"
    ...
    print(f"\n{'='*60}")
    print(f"Study: {study_name} (TTv2 - ChoppedTransferCompound)")
