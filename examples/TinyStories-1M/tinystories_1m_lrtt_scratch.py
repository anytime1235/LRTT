...
"""TinyStories-1M for Language Modeling using LRTT (Low-Rank TikiTaka Training) layers.
...
- Linear layers in transformer blocks converted to LRTT analog layers
...
RESULTS = os.path.join(os.getcwd(), "results", "TINYSTORIES_LRTT_SCRATCH")
os.makedirs(RESULTS, exist_ok=True)
WEIGHT_PATH = os.path.join(RESULTS, "tinystories_1m_lrtt_scratch_model_weight.pth")
...
class TinyStoriesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Don't pad here, use collate_fn for dynamic padding
...
def _create_6t1c_device():
    ...
    return LinearStepDevice(
        ...
        write_noise_std=0, mean_bound_reference=True,
        lifetime=lifetime, lifetime_dtod=0.1 if SIXT1C_INCLUDE_RETENTION else 0.0,
...
def create_lrtt_config():
    ...
    device_config = PythonLRTTDevice(
        rank=LRTT_RANK, transfer_every=TRANSFER_EVERY * GRAD_ACCUM_STEPS, lora_alpha=LORA_ALPHA,
        transfer_lr_scale=TRANSFER_LR_SCALE, forward_inject=False,
        reinit_mode=REINIT_MODE, reinit_gain=0.1, decay_factor=DECAY_FACTOR,
...
def create_model():
    """Create TinyStories-1M model with LRTT analog layers."""
    ...
    print("Converting Linear layers to LRTT AnalogLinear...")
    ...
    config = model.config
    print(f"\nLoaded TinyStories-1M model (LRTT):")
    ...
    print(f"  LRTT: rank={LRTT_RANK}, transfer_every={TRANSFER_EVERY}")
...
def main():
    """Train TinyStories-1M with LRTT."""
    manual_seed(SEED)
    if USE_CUDA:
        torch.cuda.manual_seed(SEED)

    wandb.init(
        project="tinystories_1m_lrtt_scratch",
        name=f"tinystories_lrtt_r{LRTT_RANK}_te{TRANSFER_EVERY}",
        config={
            "model": "TinyStories-1M-LRTT",
