...
"""TinyStories-1M for Language Modeling using TikiTaka TTv1 (TransferCompound) layers.
...
- Linear layers in transformer blocks converted to TTv1 analog layers
...
RESULTS = os.path.join(os.getcwd(), "results", "TINYSTORIES_TTV1_SCRATCH")
os.makedirs(RESULTS, exist_ok=True)
WEIGHT_PATH = os.path.join(RESULTS, "tinystories_1m_ttv1_scratch_model_weight.pth")
...
def create_ttv1_config():
    ...
    device_config = TransferCompound(
        ...
        transfer_every=TRANSFER_EVERY * GRAD_ACCUM_STEPS,  # Scale by grad_accum
...
def create_model():
    """Create TinyStories-1M model with TTv1 analog layers."""
    ...
    print("Converting Linear layers to TTv1 AnalogLinear...")
    ...
    config = model.config
    print(f"\nLoaded TinyStories-1M model (TTv1):")
    ...
    print(f"  Analog (TTv1) layers: {num_analog}")
    print(f"  TTv1: transfer_every={TRANSFER_EVERY}")
...
def main():
    """Train TinyStories-1M with TTv1."""
    manual_seed(SEED)
    if USE_CUDA:
        torch.cuda.manual_seed(SEED)

    wandb.init(
        project="tinystories_1m_ttv1_scratch",
        name=f"tinystories_ttv1_te{TRANSFER_EVERY}",
        config={
            "model": "TinyStories-1M-TTv1",
