...
"""aihwkit example: ViT with SPT+LSA for CIFAR10 using Digital/FP layers.
...
Paper FP baseline result: 29.3% error
...
RESULTS = os.path.join(os.getcwd(), "results", "VITSPTLSA_FP_SCRATCH")
os.makedirs(RESULTS, exist_ok=True)
WEIGHT_PATH = os.path.join(RESULTS, "cifar10_vitsptlsa_fp_scratch_model_weight.pth")
...
class ShiftedPatchTokenization(nn.Module):
    ...
    def __init__(self, in_channels=3, embed_dim=256, patch_size=4):
        ...
        self.in_channels = in_channels * 5  # 5 shifted versions
...
class ViT_SPT_LSA(nn.Module):
    """Vision Transformer with SPT and LSA for CIFAR-10 (Digital/FP version)."""
...
def create_model():
    """Create ViT-SPT-LSA model with digital FP layers."""
    ...
    print(f"\nCreated ViT-SPT-LSA model (Digital FP):")
...
def main():
    """Train ViT-SPT-LSA with Digital FP on CIFAR-10."""
    manual_seed(SEED)
    if USE_CUDA:
    ...
    # Initialize wandb
    wandb.init(
        project="cifar10_vitsptlsa_fp_scratch",
        name=f"vitsptlsa_fp_bs{BATCH_SIZE}_e{N_EPOCHS}_lr{LEARNING_RATE}",
        config={
            "model": "ViT-SPT-LSA",
            "optimizer": "Digital FP (SGD)",
            "dataset": "CIFAR-10",
            "image_size": IMAGE_SIZE,
            "patch_size": PATCH_SIZE,
            "embed_dim": EMBED_DIM,
    ...
    print(f"  Our FP result: {100 - best_accuracy:.1f}% error")
    print(f"{'='*60}")
