...
"""aihwkit example: ResNet18 CNN with CIFAR10 using FullAnalog layers + ImageNet pretrained weights.

CIFAR10 dataset on a ResNet18 network using FullAnalog (C-only training, no LRTT A/B)
...
RESULTS = os.path.join(os.getcwd(), "results", "RESNET_REGULAR_FULLANALOG_IMAGENET")
os.makedirs(RESULTS, exist_ok=True)
WEIGHT_PATH = os.path.join(RESULTS, "cifar10_resnet_regular_fullanalog_imagenet_model_weight.pth")
...
def main():
    """Train a PyTorch ResNet analog model with FullAnalog to classify CIFAR10."""
    # Seed
    manual_seed(SEED)
    ...
    print("\nStarting FullAnalog training on CIFAR10 with ImageNet pretrained weights...")
    print("=" * 60)
