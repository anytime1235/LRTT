# -*- coding: utf-8 -*-
"""MNIST training comparison: use_onehot=True vs False.

Quick test to compare transfer accuracy with different settings.
"""
...
def create_model(use_onehot: bool, use_sigma_delta: bool = True):
    """Create LRTT model with specified transfer settings."""
    ...
    def get_config():
        rpu_config = PythonLRTTPreset.sixt1c_ab_ideal(
            ...
            dt_batch_sec=0.0,  # No retention for faster test
            ab_update_mode="auto",
...
def run_experiment(use_onehot: bool, use_sigma_delta: bool = True):
    """Run training experiment."""
    mode_str = f"use_onehot={use_onehot}"
    if use_onehot:
        mode_str += f", sigma_delta={use_sigma_delta}"

    print(f"\n{'='*60}")
    print(f"EXPERIMENT: {mode_str}")
    print('='*60)

    train_loader, test_loader = load_mnist()
    model = create_model(use_onehot, use_sigma_delta)
...
def main():
    print("MNIST LRTT Training: use_onehot Comparison")
    ...
    # Test 1: use_onehot=False (direct weight access)
    results_direct = run_experiment(use_onehot=False)

    # Test 2: use_onehot=True with sigma_delta=True (default)
    results_onehot_sd = run_experiment(use_onehot=True, use_sigma_delta=True)

    # Test 3: use_onehot=True with sigma_delta=False
    results_onehot_simple = run_experiment(use_onehot=True, use_sigma_delta=False)

    # Summary
    print("\n" + "="*60)
    print("SUMMARY")
    print("="*60)
    print(f"{'Setting':<35} {'Test Acc':>10} {'C-only':>10}")
    print("-"*60)
    print(f"{'use_onehot=False (direct)':<35} {results_direct[-1]['test_acc']:>9.2f}% {results_direct[-1]['c_only_acc']:>9.2f}%")
    print(f"{'use_onehot=True, sigma_delta=True':<35} {results_onehot_sd[-1]['test_acc']:>9.2f}% {results_onehot_sd[-1]['c_only_acc']:>9.2f}%")
    print(f"{'use_onehot=True, sigma_delta=False':<35} {results_onehot_simple[-1]['test_acc']:>9.2f}% {results_onehot_simple[-1]['c_only_acc']:>9.2f}%")
    print("="*60)
