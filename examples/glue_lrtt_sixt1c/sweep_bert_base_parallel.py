#!/usr/bin/env python
# coding=utf-8
"""BERT-base LRTT Parallel Sweep - Multi-process on Single GPU.

Runs multiple trials in parallel on a single GPU by spawning separate processes.
Uses SQLite storage for Optuna study synchronization.

Usage:
    python sweep_bert_base_parallel.py --n_workers 6 --n_trials 200
"""
...
# Disable wandb for parallel runs to avoid conflicts
...
BATCH_SIZE = 32
NUM_EPOCHS = 3
LOGGING_STEPS = 500  # Reduce logging for parallel runs
...
OUTPUT_DIR = "/tmp/lrtt_optuna_parallel"
DB_PATH = "/data/LRTT/examples/glue_lrtt_sixt1c/optuna_bert_base.db"
...
def get_data():
    ...
    print(f"[Worker {os.getpid()}] Data loaded: {len(train_dataset)} train, {len(eval_dataset)} eval")
...
def run_single_trial(rank: int, te: int, lr: float, tlr: float, lifetime: float,
                     trial_number: int, worker_id: int) -> float:
    """Run a single trial and return accuracy."""
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    print(f"[Worker {worker_id}] Trial {trial_number}: rank={rank}, te={te}, lr={lr:.6f}, tlr={tlr:.6f}, lifetime={lifetime}")

    set_seed(SEED + trial_number)  # Different seed per trial
    ...
    try:
        ...
        training_args = TrainingArguments(
            output_dir=f"{OUTPUT_DIR}/worker{worker_id}_trial{trial_number}",
        ...
        print(f"[Worker {worker_id}] Trial {trial_number} completed: accuracy={eval_acc:.4f}")

        return eval_acc

    except Exception as e:
        print(f"[Worker {worker_id}] Trial {trial_number} failed: {e}")
        return 0.0
...
def objective(trial: optuna.Trial) -> float:
    ...
    worker_id = os.getpid() % 1000  # Simple worker ID from PID
    ...
    return run_single_trial(rank, te, lr, tlr, lifetime, trial.number, worker_id)


def worker_process(worker_id: int, storage_url: str, study_name: str, n_trials_per_worker: int):
    """Worker process that runs multiple trials."""
    print(f"[Worker {worker_id}] Starting with {n_trials_per_worker} trials")
...
def main():
    ...
    parser.add_argument("--study_name", type=str, default="lrtt_bert_base_parallel", help="Study name")
    ...
    print("=" * 60)
    print("BERT-base LRTT Parallel Sweep")
    print("=" * 60)
    print(f"Workers: {args.n_workers}")
    print(f"Total Trials: {args.n_trials}")
    print(f"Study: {args.study_name}")
    print(f"Storage: {storage_url}")
    print(f"GPU Memory per worker: ~20GB (estimated)")
    print("=" * 60)
    ...
    print(f"Trials per worker: {n_trials_per_worker} (+ {remainder} extra for first workers)")
    ...
    for i in range(args.n_workers):
        ...
        p.start()
        processes.append(p)
        time.sleep(5)  # Stagger starts to avoid data loading conflicts
    ...
    if completed_trials:
        ...
        results_file = f"{OUTPUT_DIR}/parallel_results_{timestamp}.json"
