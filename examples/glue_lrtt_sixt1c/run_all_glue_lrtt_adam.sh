...
# Settings: Same as LoRA Analog (AnalogAdam, lr=0.0002, transfer_every=1000)
...
# Common parameters - matching LoRA Analog settings
COMMON_ARGS="--model_name_or_path $MODEL_NAME \
  ...
  --analog_lr 0.0002 \
  --use_wandb True \
  --wandb_project lrtt_glue_adam"

# Task-specific epochs (same as LoRA Analog)
...
echo "Optimizer: AnalogAdam (same as LoRA Analog)"
echo "Learning Rate: 0.0002 (same as LoRA Analog)"
...
for i in "${!TASKS[@]}"; do
  ...
  python run_glue_lrtt.py \
    ...
    --output_dir ./results/$TASK/lrtt_adam_${TASK} \
    2>&1 | tee ./logs/${TASK}_lrtt_adam.log
...
echo ""
echo "========================================"
echo "All LRTT (Adam) GLUE Tasks Completed!"
echo "Time: $(date)"
echo "========================================"
