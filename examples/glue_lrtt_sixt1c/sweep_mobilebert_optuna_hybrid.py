#!/usr/bin/env python
# coding=utf-8
"""MobileBERT LRTT HYBRID REINIT Bayesian Hyperparameter Search with Optuna.

Hybrid reinit mode: A=0 (hard reset), B=B*decay_factor (with decay_factor=1.0, B unchanged)
...
Usage:
    # Single GPU
    nohup python sweep_mobilebert_optuna_hybrid.py --n_trials 200 > optuna_hybrid_sweep.log 2>&1 &

    # Parallel with multiple workers (each worker on different GPU)
    nohup python sweep_mobilebert_optuna_hybrid.py --n_trials 200 --n_jobs 4 > optuna_hybrid_sweep.log 2>&1 &

    # Resume from existing study
    nohup python sweep_mobilebert_optuna_hybrid.py --n_trials 200 --study_name my_study --storage sqlite:///optuna.db > optuna_hybrid_sweep.log 2>&1 &
"""
...
TASK_NAME = "sst2"
MAX_SEQ_LENGTH = 128
BATCH_SIZE = 32   # Same as lora_on_analog_hardware baseline
NUM_EPOCHS = 15   # Same as lora_on_analog_hardware baseline
LOGGING_STEPS = 100
...
WANDB_PROJECT = "lrtt-mobilebert-hybrid-reinit"
OUTPUT_DIR = "/tmp/lrtt_optuna_hybrid_results"
...
def main():
    ...
    study_name = args.study_name or f"lrtt_mobilebert_hybrid_{timestamp}"

    print("=" * 60)
    print("MobileBERT LRTT HYBRID REINIT Bayesian Search (Optuna)")
    print("=" * 60)
    ...
    print(f"Reinit Mode: HYBRID (A=0 hard reset, B unchanged)")
    print("-" * 60)
    print(f"Ranks: {RANKS}")
