...
# coding=utf-8
"""LRTT Bayesian Optimization for SQuAD - RANK 8, O-only (15 epoch).

15-epoch sweep with cosine+minLR scheduler and early stopping.
Targets attention.output.dense (O projection) only - 24 layers.
Search space same as K-only baseline for direct comparison.
"""
...
WANDB_PROJECT = "lrtt-squad-rank8-O-only-15ep-sweep"
OUTPUT_DIR = "/root/results/SQUAD/O_only"
...
def run_task_sweep(task_name: str, device: torch.device, results_dir: str, n_trials: int = 10, n_jobs: int = 1):
    ...
    all_trials_file = os.path.join(results_dir, f"{task_name}_rank{RANK}_O_only_15ep_all_trials.json")
...
def main():
    ...
    print("="*60)
    print("LRTT Bayesian Optimization - SQuAD RANK 8 (O-only, 15ep)")
    print("="*60)
