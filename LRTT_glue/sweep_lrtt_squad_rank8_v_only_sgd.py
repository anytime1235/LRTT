...
# coding=utf-8
"""LRTT Bayesian Optimization for SQuAD - RANK 8, Value-only (AnalogSGD).

Based on sweep_lrtt_squad_rank8_v_only.py but using AnalogSGD optimizer.
Default trial seeded from Adam V-only best: lr=0.0396, t_lr=3.12e-4, te=193.
"""
...
# Default LRTT config (seeded from Adam V-only best)
DEFAULT_LRTT_PARAMS = {
...
WANDB_PROJECT = "lrtt-squad-rank8-v-only-sgd-sweep"
...
def run_task_sweep(task_name: str, device: torch.device, results_dir: str, n_trials: int = 10, n_jobs: int = 1):
    ...
    all_trials_file = os.path.join(results_dir, f"{task_name}_rank{RANK}_v_only_sgd_all_trials.json")
...
def main():
    ...
    print("="*60)
    print("LRTT Bayesian Optimization - SQuAD RANK 8 (Value-only, AnalogSGD)")
    print("="*60)
