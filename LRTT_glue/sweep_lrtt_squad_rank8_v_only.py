...
# coding=utf-8
"""LRTT Bayesian Optimization for SQuAD - RANK 8, Value-only.

Based on sweep_lrtt_squad_rank8.py but only converts Value layers.
Target modules: ["value"] instead of ["query", "key", "value"].
"""
...
WANDB_PROJECT = "lrtt-squad-rank8-v-only-sweep"
...
def run_task_sweep(task_name: str, device: torch.device, results_dir: str, n_trials: int = 10, n_jobs: int = 1):
    ...
    all_trials_file = os.path.join(results_dir, f"{task_name}_rank{RANK}_v_only_all_trials.json")
...
def main():
    ...
    print("="*60)
    print("LRTT Bayesian Optimization - SQuAD RANK 8 (Value-only)")
    print("="*60)
