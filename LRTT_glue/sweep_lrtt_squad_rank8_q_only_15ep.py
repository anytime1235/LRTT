...
# coding=utf-8
"""LRTT Bayesian Optimization for SQuAD - RANK 8, Query-only (15 epoch).

15-epoch sweep with cosine+minLR scheduler and early stopping.
Search space narrowed around Adam 3-epoch best: lr=0.0139, t_lr=1.47e-4, te=5032.
"""
...
# Default LRTT config (seeded from Adam 3-epoch Q-only best)
DEFAULT_LRTT_PARAMS = {
...
WANDB_PROJECT = "lrtt-squad-rank8-q-only-15ep-sweep"
...
def run_task_sweep(task_name: str, device: torch.device, results_dir: str, n_trials: int = 10, n_jobs: int = 1):
    ...
    all_trials_file = os.path.join(results_dir, f"{task_name}_rank{RANK}_q_only_15ep_all_trials.json")
...
def main():
    ...
    print("="*60)
    print("LRTT Bayesian Optimization - SQuAD RANK 8 (Query-only, 15ep)")
    print("="*60)
