...
# coding=utf-8
"""LRTT Bayesian Optimization for SQuAD - RANK 8, Bottleneck-only (15 epoch).

15-epoch sweep with cosine+minLR scheduler and early stopping.
Targets bottleneck dense layers only - 72 layers (MobileBERT specific).
"""
...
WANDB_PROJECT = "lrtt-squad-rank8-bottleneck-only-15ep-sweep"
OUTPUT_DIR = "/root/results/SQUAD/Bottleneck_only"
...
def run_task_sweep(task_name: str, device: torch.device, results_dir: str, n_trials: int = 10, n_jobs: int = 1):
    ...
    all_trials_file = os.path.join(results_dir, f"{task_name}_rank{RANK}_bottleneck_only_15ep_all_trials.json")
...
def main():
    ...
    print("="*60)
    print("LRTT Bayesian Optimization - SQuAD RANK 8 (Bottleneck-only, 15ep)")
    print("="*60)
