...
# coding=utf-8
"""LRTT Bayesian Optimization for SQuAD - RANK 8, FFN-only (15 epoch).

15-epoch sweep with cosine+minLR scheduler and early stopping.
Targets FFN layers (intermediate.dense, output.dense, ffn.*.dense) - 216 layers.
Excludes attention projections (Q/K/V/O) and bottleneck layers.
"""
...
N_TRIALS = 50
NUM_EPOCHS = 15
TARGET_MODULES = ["intermediate.dense", "output.dense", "ffn"]
# Excludes: attention.output.dense (O proj), bottleneck.*.dense
# "ffn" matches ffn.0/1/2.intermediate.dense and ffn.0/1/2.output.dense
# We use EXCLUDE_PATTERNS to filter out non-FFN dense layers
EXCLUDE_PATTERNS = ["bottleneck", "attention.output.dense"]
...
WANDB_PROJECT = "lrtt-squad-rank8-ffn-only-15ep-sweep"
OUTPUT_DIR = "/root/results/SQUAD/FFN_only"
...
def is_target_layer(name: str) -> bool:
    """Check if layer matches FFN target (exclude attention O-proj and bottleneck)."""
...
def create_glue_model(task_name: str, params: Dict, device: torch.device) -> nn.Module:
    """Create GLUE model with LRTT (FFN-only)."""
    ...
    exclude = [name for name in all_linear if not is_target_layer(name)]
    ...
    for name, param in model.named_parameters():
        param.requires_grad = is_target_layer(name) or "classifier" in name
...
def create_squad_model(params: Dict, device: torch.device) -> nn.Module:
    """Create SQuAD model with LRTT (FFN-only)."""
    ...
    for name, param in model.named_parameters():
        param.requires_grad = is_target_layer(name) or "qa_outputs" in name
...
def run_task_sweep(task_name: str, device: torch.device, results_dir: str, n_trials: int = 10, n_jobs: int = 1):
    ...
    all_trials_file = os.path.join(results_dir, f"{task_name}_rank{RANK}_ffn_only_15ep_all_trials.json")
...
def main():
    ...
    print("="*60)
    print("LRTT Bayesian Optimization - SQuAD RANK 8 (FFN-only, 15ep)")
    print("="*60)
