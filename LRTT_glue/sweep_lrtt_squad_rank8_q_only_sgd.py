...
# coding=utf-8
"""LRTT Bayesian Optimization for SQuAD - RANK 8, Query-only (AnalogSGD).

Based on sweep_lrtt_squad_rank8_q_only.py but using AnalogSGD optimizer.
Default trial seeded from Adam Q-only best: lr=0.0139, t_lr=1.47e-4, te=5032.
"""
...
MOMENTUM_MIN, MOMENTUM_MAX = 0.0, 0.99             # SGD momentum
...
# Default LRTT config (seeded from Adam Q-only best)
DEFAULT_LRTT_PARAMS = {
...
WANDB_PROJECT = "lrtt-squad-rank8-q-only-sgd-sweep"
...
def run_trial(trial: optuna.Trial, task_name: str, train_loader, eval_loader,
    ...
    try:
        ...
        # Create optimizer with AnalogSGD
        optimizer = AnalogSGD(model.parameters(), lr=params["learning_rate"], momentum=params["momentum"])
...
def run_task_sweep(task_name: str, device: torch.device, results_dir: str, n_trials: int = 10, n_jobs: int = 1):
    ...
    all_trials_result = {
        ...
        "search_space": {
            ...
            "momentum": {"min": MOMENTUM_MIN, "max": MOMENTUM_MAX, "scale": "linear"},
        },
        "fixed_params": {
            "rank": RANK,
            "lora_alpha": LORA_ALPHA,
    ...
    all_trials_file = os.path.join(results_dir, f"{task_name}_rank{RANK}_q_only_sgd_all_trials.json")
...
def main():
    ...
    print("="*60)
    print("LRTT Bayesian Optimization - SQuAD RANK 8 (Query-only, AnalogSGD)")
    print("="*60)
    ...
    print(f"  momentum: [{MOMENTUM_MIN}, {MOMENTUM_MAX}] (linear)")
    print()
