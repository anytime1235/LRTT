...
# coding=utf-8
"""LRTT Bayesian Optimization for SQuAD - RANK 8, Value-only (15 epoch).
...
WANDB_PROJECT = "lrtt-squad-rank8-v-only-15ep-sweep"
...
def run_task_sweep(task_name: str, device: torch.device, results_dir: str, n_trials: int = 10, n_jobs: int = 1):
    ...
    all_trials_file = os.path.join(results_dir, f"{task_name}_rank{RANK}_v_only_15ep_all_trials.json")
...
def main():
    ...
    print("="*60)
    print("LRTT Bayesian Optimization - SQuAD RANK 8 (Value-only, 15ep)")
    print("="*60)
