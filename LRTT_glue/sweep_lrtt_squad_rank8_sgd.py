...
# coding=utf-8
"""LRTT Bayesian Optimization for SQuAD - RANK 8 (AnalogSGD).

Based on sweep_lrtt_squad_rank8.py but using AnalogSGD optimizer.
Default trial seeded from Adam QKV best: lr=0.0142, t_lr=3.80e-4, te=100.
"""
...
# Default LRTT config (seeded from Adam QKV best)
DEFAULT_LRTT_PARAMS = {
...
WANDB_PROJECT = "lrtt-squad-rank8-sgd-sweep"
...
def run_task_sweep(task_name: str, device: torch.device, results_dir: str, n_trials: int = 10, n_jobs: int = 1):
    ...
    all_trials_file = os.path.join(results_dir, f"{task_name}_rank{RANK}_sgd_all_trials.json")
...
def main():
    ...
    print("="*60)
    print("LRTT Bayesian Optimization - SQuAD RANK 8 (AnalogSGD)")
    print("="*60)
    print(f"Mode: {MODE}")
    if MODE == "sixt1c_lora":
