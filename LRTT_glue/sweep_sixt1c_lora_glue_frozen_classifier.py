#!/usr/bin/env python
# coding=utf-8
"""Sixt1c-LoRA Bayesian Optimization for GLUE tasks (AnalogSGD).

Architecture (FIXED: TorchInferenceRPUConfig for base_layer):
- Uses PEFT LoRA (base_layer + lora_A + lora_B structure)
- base_layer: TorchInferenceRPUConfig (inference only, frozen, NO updates)
- lora_A/B: SingleRPUConfig + LinearStepDevice 6T1C (trainable, analog)
- base_layer: ALL NOISE = 0 (is_perfect=True, deterministic)
- lora_A/B: noise_management=ABS_MAX, bound_management=ITERATIVE
- mult_noise=False, reinit_mode=decay, AnalogSGD optimizer
...
# Standardized sixt1c config imports
...
def create_glue_model(task_name: str, device: torch.device, target_modules: List[str],
    ...
    # Step 3: Smart Conversion with SingleRPUConfig (consistent)
    if fp_lora:
        print("\n[FP-LoRA Mode] Skipping analog conversion - keeping LoRA digital")
    else:
    ...
    # - base_layer: already frozen (requires_grad=False in smart_conversion)
    # - lora analog: already set (requires_grad=True in smart_conversion)
    # - classifier: FROZEN (requires_grad=False) - MODIFIED FOR EXPERIMENT
    # - lora digital params: set trainable
...
def main():
    ...
    parser.add_argument("--fp_lora", action="store_true",
                       help="Keep LoRA adapters as digital FP (skip sixt1c conversion)")
    ...
    mode_prefix = "fp_lora" if args.fp_lora else "sixt1c_lora"
    results_dir = os.path.join(OUTPUT_DIR, f"{mode_prefix}_bias_frozen_{timestamp}")
    os.makedirs(results_dir, exist_ok=True)

    lora_desc = "Digital FP (trainable)" if args.fp_lora else "6T1C sixt1c (trainable, analog)"

    print("="*60)
    print(f"{'FP-LoRA' if args.fp_lora else 'Sixt1c-LoRA'} Bayesian Optimization - GLUE")
    print("="*60)
    print(f"Architecture: Standardized Config (PEFT LoRA)")
    print(f"Mode: {'FP_lora (digital adapters)' if args.fp_lora else 'sixt1c_lora (6T1C analog adapters)'}")
    ...
    print()
    # Determine actual lora_alpha value
    ...
    print(f"  architecture: Smart Conversion (SingleRPUConfig)")
    print(f"  base_layer: SoftBounds (frozen, analog)")
    print(f"  lora_A/B: {lora_desc}")
