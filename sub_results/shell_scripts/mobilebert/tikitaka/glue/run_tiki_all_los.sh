#!/bin/bash
# TikiTaka v1 - ALL target layer + learn_out_scaling=False
...
LOG=${LOG_DIR}/run_all_tpe_all.log
SUFFIX="_tpe"

mkdir -p ${LOG_DIR}

echo "==========================================" | tee -a $LOG
echo "TikiTaka v1 ALL + learn_out_scaling=False" | tee -a $LOG
echo "==========================================" | tee -a $LOG
echo "Start: $(date)" | tee -a $LOG
echo "" | tee -a $LOG
echo "Tasks: RTE(30),MRPC(30),STS-B(30),CoLA(30),SST-2(30),QNLI(10),QQP(10),MNLI(10)" | tee -a $LOG
echo "Config: AnalogSGD, no-wd, no-momentum, no-nesterov, TPE, all, f_lr=(0.001,0.5)" | tee -a $LOG
echo "" | tee -a $LOG
