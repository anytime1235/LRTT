# -*- coding: utf-8 -*-
"""2-Stage Optuna HPO for ALBERT + SQuAD with LoRA-LRTT.

Stage 1: Pretrain qa_outputs + LayerNorm (run pretrain_qa_full.py first)
Stage 2: Sweep LoRA-LRTT HPs with frozen qa_outputs + LayerNorm (this script)

Requires: /data/classifier_ckpt/squad_v1_adam_full/ckpt.pt from pretrain_qa_full.py

This script automatically:
  - Loads pretrained qa_outputs + LayerNorm from Stage 1 checkpoint
  - Freezes all digital params (qa_outputs, LayerNorm)
  - Trains only LRTT analog A/B tiles + out_scaling
  - Uses remaining epoch budget for Stage 2

Usage:
    python optuna_albert_squad_lora_2stage.py --n-trials 50
    python optuna_albert_squad_lora_2stage.py --visualize
    python optuna_albert_squad_lora_2stage.py --n-trials 50 --optimizer AnalogSGD --lora-target attn
...
DEFAULT_STUDY_NAME = "albert_squad_lora2s_main"

# 2-Stage config
PRETRAIN_CKPT_DIR = "/data/classifier_ckpt"
PRETRAIN_CKPT = None  # set via --pretrain-ckpt or auto-detected (squad_v1_adam_full)
...
# Out-scaling only mode: freeze ALL tiles, train only out_scaling + combined_out_scaling
OSCALE_ONLY = False
FIXED_LR_LIST = None  # set via --fix-lr
FIXED_OSCALE_LR = 1e-3  # fixed LR for out_scaling digital params (set via --oscale-lr)
LORA_TARGET_MODULES = {
...
def create_model(params):
    ...
    # Load pretrained qa_outputs + LayerNorm from Stage 1
    ...
    if not os.path.exists(ckpt_path):
        raise FileNotFoundError(
            ...
            f"Run: python /data/pretrain_qa_full.py"
        )
    ...
    print(f"  [Stage 1] {ckpt.get('metric_name','?')}: {ckpt.get('metric_value', 0):.4f} "
          f"(epoch {ckpt.get('epochs', '?')}, lr={ckpt.get('lr', '?')})")
    ...
    if OSCALE_ONLY:
        # ============================================================
        # OSCALE_ONLY mode: freeze ALL tiles, train only out_scaling
        # + combined_out_scaling for all analog layers (target + NT)
        # ============================================================
        # Freeze LRTT target tiles too (NT already frozen above)
        def _lrtt_noop_update(x_input, d_input, *args, **kwargs):
            return None
        for mod_name, m in model.named_modules():
            if isinstance(m, AnalogLinear) and is_lrtt_target(mod_name):
                for tile in m.analog_tiles():
                    tile.update = _lrtt_noop_update
        print(f"  [OSCALE_ONLY] All analog tiles frozen (noop update)")

        # Train ALL out_scaling + combined_out_scaling (target + non-target)
        oscale_params = []
        for name, param in model.named_parameters():
            if isinstance(param, AnalogContext):
                param.requires_grad = True  # gradient flow through analog forward
            elif "out_scaling" in name or "combined_out_scaling" in name:
                param.requires_grad = True
                oscale_params.append((name, param.numel()))
            else:
                param.requires_grad = False

        print(f"  [OSCALE_ONLY] Trainable out_scaling params:")
        for pname, pcount in oscale_params:
            print(f"    {pname}: {pcount}")
    else:
        # ============================================================
        # Normal Stage 2: LRTT A/B tiles + target out_scaling trainable
        # qa_outputs + LayerNorm frozen (pretrained in Stage 1)
        # ============================================================
        lrtt_layer_patterns = [t.split(".")[-1] for t in [n for n in all_linear_names if is_lrtt_target(n)]]
        for name, param in model.named_parameters():
            if isinstance(param, AnalogContext):
                param.requires_grad = True  # required for analog tile update
            elif "out_scaling" in name and any(p in name for p in lrtt_layer_patterns):
                # learn_out_scaling only for LRTT target layers
                param.requires_grad = True
            else:
    ...
    if OSCALE_ONLY:
        print(f"  Mode: OSCALE_ONLY (all tiles frozen, only out_scaling trainable)")
...
def objective(trial, train_loader, eval_features, eval_examples, tokenizer):
    ...
    # Hyperparameters
    if OSCALE_ONLY:
        # OSCALE_ONLY: only learning_rate matters (tiles frozen)
        if FIXED_LR_LIST:
            learning_rate = trial.suggest_categorical('learning_rate', FIXED_LR_LIST)
        else:
            learning_rate = trial.suggest_float('learning_rate', 0.001, 1.0, log=True)
        lora_alpha_val = 1.0  # doesn't matter (A=0, LoRA contribution=0)
        target_ab_lr = learning_rate  # placeholder
    else:
        # out_scaling LR: fixed (FIXED_OSCALE_LR), used as optimizer base LR
        learning_rate = FIXED_OSCALE_LR
        if FIXED_LR_LIST:
            lora_alpha_val = trial.suggest_categorical('lora_alpha', FIXED_LR_LIST)  # reuse for grid
            target_ab_lr = trial.suggest_categorical('target_ab_lr', FIXED_LR_LIST)
        else:
            lora_alpha_val = trial.suggest_float('lora_alpha', 0.001, 1.0, log=True)
            target_ab_lr = trial.suggest_float('target_ab_lr', 0.001, 1.0, log=True)

    # LRTT parameters: skip sweep if --no-transfer (A/B frozen, no transfer happens)
    if OPT_CONFIG['no_transfer'] or OSCALE_ONLY:
        transfer_lr = 0.1
    ...
    print(f"\n{'='*70}")
    print(f"Trial {trial.number} Starting {'[OSCALE_ONLY]' if OSCALE_ONLY else ''}")
    print(f"{'='*70}")
    if OSCALE_ONLY:
        print(f"  lr={learning_rate:.2e} (out_scaling only)")
    else:
        print(f"  rank={rank}, lora_alpha={lora_alpha:.4f}, oscale_lr={learning_rate:.2e} (fixed)")
    ...
    try:
        ...
        # Save out_scaling checkpoint in OSCALE_ONLY mode
        if OSCALE_ONLY and model is not None:
            oscale_state = {}
            for pname, param in model.named_parameters():
                if "out_scaling" in pname or "combined_out_scaling" in pname:
                    oscale_state[pname] = param.detach().cpu().clone()
            oscale_dir = os.path.join(PRETRAIN_CKPT_DIR, f"squad_v1_oscale_{LORA_TARGET}")
            os.makedirs(oscale_dir, exist_ok=True)
            oscale_path = os.path.join(oscale_dir, f"oscale_trial_{trial.number}.pt")
            torch.save({
                'oscale_state': oscale_state,
                'f1': best_f1,
                'trial': trial.number,
            ...
            print(f"  [OSCALE] Saved {len(oscale_state)} params to {oscale_path}")
...
def main():
    global BATCH_SIZE, N_EPOCHS, WARMUP_STEPS, WARM_ALPHA, TRANSFER_METHOD, AB_DEVICE, IO_NOISE, LORA_TARGET, HEAD_LAYER, AB_PERFECT_IO, COMBINED_OUT_SCALING, CONVERT_NONTARGET, PRETRAIN_CKPT, OSCALE_ONLY, FIXED_LR_LIST, FIXED_OSCALE_LR

    parser = argparse.ArgumentParser(description="2-Stage Optuna sweep for ALBERT SQuAD LoRA-LRTT")
    ...
    parser.add_argument('--oscale-only', action='store_true',
                        help='Out-scaling calibration mode: freeze ALL tiles, train only out_scaling + combined_out_scaling')
    parser.add_argument('--fix-lr', type=float, nargs='+', default=None,
                        help='Fix learning_rate to these values (GridSampler). e.g. --fix-lr 0.1 0.01 0.001')
    parser.add_argument('--oscale-lr', type=float, default=1e-3,
                        help='Fixed LR for out_scaling digital params (default: 1e-3)')
    ...
    OSCALE_ONLY = args.oscale_only
    FIXED_LR_LIST = args.fix_lr
    FIXED_OSCALE_LR = args.oscale_lr

    # Auto-generate study name based on config (includes batch size)
    study_name = args.study_name or f"albert_squad_lrtt2s_bs{BATCH_SIZE}_{get_study_name_suffix()}"
    ...
    # Compute warmup from ratio (always use ratio, ignore --warmup-steps)
    ...
    # Sampler selection
    if args.fix_lr and OSCALE_ONLY:
        # Grid over fixed LR values for oscale calibration
        grid_search_space = {"learning_rate": args.fix_lr}
        sampler = optuna.samplers.GridSampler(grid_search_space)
        args.n_trials = len(args.fix_lr)
        print(f"[OSCALE] GridSampler: LR={args.fix_lr}")
    elif args.fix_lr:
        grid_search_space = {
            "learning_rate": args.fix_lr,
            "lora_alpha": [0.06],
            "target_ab_lr": [0.01],
        }
        sampler = optuna.samplers.GridSampler(grid_search_space)
        print(f"GridSampler: LR={args.fix_lr}")
    else:
    ...
    # Enqueue seed trial (only for TPE mode without oscale)
    if not args.fix_lr and not OSCALE_ONLY and len(study.trials) == 0:
        study.enqueue_trial({
    ...
    # Copy best trial's oscale checkpoint as oscale_best.pt
    if OSCALE_ONLY and complete_trials:
        import shutil
        best_trial = study.best_trial
        oscale_dir = os.path.join(PRETRAIN_CKPT_DIR, f"squad_v1_oscale_{LORA_TARGET}")
        src = os.path.join(oscale_dir, f"oscale_trial_{best_trial.number}.pt")
        dst = os.path.join(oscale_dir, "oscale_best.pt")
        if os.path.exists(src):
            shutil.copy(src, dst)
            print(f"\n[OSCALE] Best trial {best_trial.number} (F1={best_trial.value:.2f}%) "
                  f"saved to {dst}")
