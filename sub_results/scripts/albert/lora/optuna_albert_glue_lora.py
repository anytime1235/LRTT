# -*- coding: utf-8 -*-
"""Optuna hyperparameter sweep for ALBERT + GLUE with LoRA-LRTT.

LoRA mode: forward_inject=True, transfer disabled (transfer_every=10^7)
Searches: learning_rate, lora_alpha, target_ab_lr

Usage:
    python optuna_albert_glue_lora.py --task sst2 --n-trials 50
    python optuna_albert_glue_lora.py --task sst2 --visualize
    python optuna_albert_glue_lora.py --task sst2 --n-trials 50 --optimizer AnalogSGD --no-wd --no-momentum --no-nesterov --batch-size 64 --epochs 5 --warmup-ratio 0.1 --lora-target attn

All flags:
    python optuna_albert_glue_lora.py \
        ...
        --no-convert-nontarget      # Keep non-target layers digital (default)
...
DEFAULT_STUDY_NAME = "albert_glue_lora_main"
...
CONVERT_NONTARGET = False  # default off (keep digital)
LORA_TARGET_MODULES = {
...
def create_model(params):
    ...
    for name, param in model.named_parameters():
        if "tile_a" in name or "tile_b" in name:
            ...
            # out_scaling: TRAINABLE only if LEARN_OUT_SCALING is True
...
def objective(trial, train_loader, eval_loader, tokenizer, study=None):
    ...
    # Hyperparameters - sweep lora_alpha, target_ab_lr, learning_rate
    # learning_rate: digital params (LN, classifier, out_scaling)
    # sweep range: per-task baseline best LR × [1/10, 3]
    ...
    try:
        ...
        for epoch in range(1, N_EPOCHS + 1):
            ...
            # Cross-trial pruning & Optuna pruning disabled for correlation analysis
...
def main():
    global TASK_NAME, BATCH_SIZE, N_EPOCHS, MAX_SEQ_LENGTH, WARMUP_RATIO, WARM_ALPHA, TRANSFER_METHOD, AB_DEVICE, IO_NOISE, AB_PERFECT_IO, BACKWARD_PERFECT, COMBINED_OUT_SCALING, LORA_TARGET, HEAD_LAYER, RESULTS, CONVERT_NONTARGET

    parser = argparse.ArgumentParser(description="Optuna sweep for ALBERT GLUE LoRA-LRTT")
    ...
    parser.add_argument('--backward-perfect', action='store_true',
    ...
    parser.add_argument('--warm-alpha', action='store_true', default=WARM_ALPHA,
                        help='Enable warm-up for lora_alpha (linear: 0 -> target over warmup steps)')
    parser.add_argument('--no-warm-alpha', dest='warm_alpha', action='store_false',
                        help='Disable warm-up for lora_alpha')
    ...
    parser.add_argument('--no-convert-nontarget', dest='convert_nontarget', action='store_false',
                        help='Keep non-target layers digital (default)')
    ...
    TASK_NAME = args.task
    RESULTS = f"/data/results/Analoglora_all/{TASK_NAME}"
    ...
    N_EPOCHS = args.epochs if args.epochs != 20 else task_cfg.get("epochs", 20)
    ...
    # No seed trials - let TPE explore uniformly from startup
