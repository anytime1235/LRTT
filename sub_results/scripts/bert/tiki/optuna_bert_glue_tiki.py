# -*- coding: utf-8 -*-
"""Optuna hyperparameter sweep for BERT-base + GLUE with TikiTaka v1.

Usage:
    python optuna_bert_glue_tiki.py --task sst2 --n-trials 50
    python optuna_bert_glue_tiki.py --task sst2 --visualize
    python optuna_bert_glue_tiki.py --task cola --n-trials 50 --optimizer AnalogSGD --no-wd --no-momentum --no-nesterov

All flags:
    python optuna_bert_glue_tiki.py \
...
# Number of columns per attention tile (BERT-base Q/K/V/W_O = Linear(768,768))
ATTN_TILE_COLUMNS = 768
...
def get_study_name_suffix():
    ...
    if TARGET_LAYERS is not None:
        layer_str = "_".join(str(i + 1) for i in sorted(TARGET_LAYERS))  # back to 1-indexed for readability
...
def create_model(params):
    """Create BERT-base classification model with selective TikiTaka v1 analog layers.

    Architecture:
        QKV mode:  48 LRTT (attention), 24 NT Analog (ffn), 1-2 Digital
        FFN mode:  24 LRTT (ffn),       48 NT Analog (attention), 1-2 Digital
        ALL mode:  72 LRTT (attn+ffn),   0 NT Analog, 1-2 Digital
        Digital = classifier + pooler (always)
    ...
    # Always digital (never analog): classifier + pooler
    ...
    def is_tikitaka_target(layer_name):
        ...
        if LORA_TARGET == "none":
            ...
            return cat in ('attention', 'ffn')  # no bottleneck in BERT-base
        elif LORA_TARGET in ("qonly", "konly", "vonly"):
    ...
    # --- Pass 1: Convert target layers to TikiTaka or IdealDevice ---
    tikitaka_count = 0
    ideal_count = 0
    if tikitaka_layers and LORA_TARGET != "none" and TARGET_IDEAL:
    ...
    if non_target_encoder_layers and not OPT_CONFIG.get('nontarget_digital', False):
        ...
        single_rpu_count = sum(1 for m in model.modules() if isinstance(m, AnalogLinear)) - tikitaka_count - ideal_count

        # Freeze Single RPU tile weights via noop update hook
        # (skip IdealDevice tiles — they are trainable)
        def _frozen_noop_update(x_input, d_input, *args, **kwargs):
            return None
        for m in model.modules():
            if isinstance(m, AnalogLinear):
                for tile in m.analog_tiles():
                    if isinstance(tile.rpu_config, SingleRPUConfig) and not isinstance(tile.rpu_config.device, IdealDevice):
    ...
    print(f"  TikiTaka: {tikitaka_count}, Ideal: {ideal_count}, NT Analog: {single_rpu_count}, "
          f"Total analog: {tikitaka_count + ideal_count + single_rpu_count}, Total params: {total_params:,}")
...
def objective(trial, train_loader, eval_loader):
    ...
    # Learning rate: override or fixed per-task base LR
    _base_lr = OPT_CONFIG.get('lr_override', None) or TASK_TO_LR.get(TASK_NAME, 0.001)
    ...
    # desired_bl: grid, sweep, or fixed
    ...
    _cls_lr_str = f", classifier_lr={OPT_CONFIG['classifier_lr']:.2e}" if OPT_CONFIG.get('classifier_lr') else ""
    ...
    try:
        ...
        if TARGET_IDEAL or OPT_CONFIG.get('nontarget_ideal', False):
            # IdealDevice: separate LR for analog tiles vs digital params
            _cls_lr = OPT_CONFIG.get('classifier_lr', None)
...
def main():
    global BATCH_SIZE, N_EPOCHS, WARMUP_RATIO, LORA_TARGET, HEAD_LAYER, TASK_NAME, MAX_SEQ_LENGTH, TARGET_IDEAL

    parser = argparse.ArgumentParser(description="Optuna sweep for BERT-base GLUE TikiTaka v1")
    ...
    parser.add_argument('--analog-only-warmup', action='store_true', default=True,
                        help='Apply LR warmup only to analog tile params; digital params start at full LR (default: True)')
    ...
    parser.add_argument('--lr', type=float, default=None,
                        help='Override per-task learning rate (e.g. --lr 1e-4)')
    parser.add_argument('--classifier-lr', type=float, default=None,
                        help='Separate LR for digital params (classifier, LayerNorm). Analog tiles use --lr.')
    ...
    study_name = args.study_name or f"bert_tiki_{TASK_NAME}_{timestamp}"
