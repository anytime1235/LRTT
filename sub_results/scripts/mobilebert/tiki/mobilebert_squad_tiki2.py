# -*- coding: utf-8 -*-
"""MobileBERT + SQuAD with TikiTaka v2.

Single-run training script for MobileBERT on SQuAD using TikiTaka v2 analog layers.
Converts target attention/FFN layers to analog; all other layers remain digital.

Based on mobilebert_squad_tiki.py, converted from TikiTaka v1 to TikiTaka v2
(ChoppedTransferCompound with 2-device: A tile (LinearStepDevice/6T1C) +
 C tile (SoftBoundsDevice) + digital chopped buffer).

Key differences from TikiTaka v1:
    - TransferCompound -> ChoppedTransferCompound
    - Added chopper: in_chop_prob, in_chop_random
    - Added auto-scaling: auto_scale, auto_granularity, auto_momentum
    - Added buffer: buffer_granularity
    - units_in_mbatch=False (mat-vec units instead of m_batch)
    - transfer_update with desired_bl=1
...
Inline flags (edit directly in script):
    ...
    TRANSFER_EVERY = 20              # Transfer interval (mat-vec units)
    TRANSFER_LR = 0.05               # Transfer learning rate
    FAST_LR = 2.0                    # Fast tile learning rate
    AUTO_GRANULARITY = 169.0         # Buffer auto-granularity
    IN_CHOP_PROB = 0.06              # Chopper switching probability
...
# Paths
RESULTS = os.path.join(os.getcwd(), "results", "MOBILEBERT_SQUAD_TIKI2")
os.makedirs(RESULTS, exist_ok=True)
WEIGHT_PATH = os.path.join(RESULTS, "mobilebert_squad_tiki2_model_weight.pth")
...
# TikiTaka v2 parameters (ChoppedTransferCompound defaults from aihwkit)
TRANSFER_EVERY = 1              # mat-vec units (units_in_mbatch=False), aihwkit default=1
TRANSFER_LR = 1.0               # Transfer learning rate, aihwkit default=1.0
FAST_LR = 1.0                   # Fast tile learning rate, aihwkit default=1.0
AUTO_GRANULARITY = 0.0           # Buffer auto-granularity scaling, aihwkit default=0.0
IN_CHOP_PROB = 0.1               # Input chopper switching probability, aihwkit default=0.1
AUTO_MOMENTUM = 0.99             # Auto-scale momentum, aihwkit default=0.99
BUFFER_GRANULARITY = 1.0         # Buffer granularity, aihwkit default=1.0
AUTO_SCALE = True                # Auto-scale gradient, aihwkit preset default=True
...
# WandB
WANDB_PROJECT = "mobilebert-squad-tiki2"
os.environ["WANDB_MODE"] = "offline"


# =============================================================================
# TikiTaka v2 Device Functions
# =============================================================================

def _create_a_device():
    """Create A tile: 6T1C LinearStepDevice (fast, noisy).

    Identical to TikiTaka v1's A tile config.
    """
...
def _create_c_device():
    """Create C tile: noise-free SoftBoundsDevice (slow, accurate).

    Identical to TikiTaka v1's B tile config.
    """
...
def create_tikitaka_v2_config():
    """Create TikiTaka v2 RPU configuration for analog layers.

    Uses ChoppedTransferCompound with 2 devices + digital chopped buffer:
        A tile (LinearStepDevice/6T1C) - fast, noisy accumulator
        C tile (SoftBoundsDevice) - slow, accurate storage

    Key differences from v1 (TransferCompound):
        - ChoppedTransferCompound: adds chopper + digital buffer
        - in_chop_prob/in_chop_random: random sign changes to reduce bias
        - auto_scale/auto_granularity: dynamic gradient scaling
        - units_in_mbatch=False: transfer_every in mat-vec units
        - transfer_update: desired_bl=1 for single-pulse transfer
    """
    ...
    rpu_config = UnitCellRPUConfig(
        device=ChoppedTransferCompound(
            unit_cell_devices=[a_device, c_device],
            # Transfer settings
            transfer_every=TRANSFER_EVERY,
            units_in_mbatch=False,     # v2: mat-vec units (v1 used True/m_batch)
...
def create_model():
    """Create MobileBERT QA model with selective TikiTaka v2 analog layers.

    Architecture:
        - Target layers (based on LORA_TARGET) -> TikiTaka v2 Analog
    ...
    TikiTaka v2 layers have:
        - A tile (fast): trained via chopped gradient updates
        - C tile (slow): receives periodic buffered transfers from A
        - Digital buffer with chopper for gradient accumulation
    ...
    if LORA_TARGET == "none":
        ...
        tiki2_config = create_tikitaka_v2_config()
        model = convert_to_analog(model, tiki2_config, exclude_modules=exclude_modules)
    ...
    print(f"\nCreated MobileBERT model (TikiTaka v2):")
    print(f"  Model: {MODEL_NAME}")
    print(f"  Total params: {total_params:,}, Trainable: {num_params:,}")
    print(f"  Analog layers: {num_analog}")
    print(f"  TikiTaka v2 config: transfer_every={TRANSFER_EVERY}, "
          f"transfer_lr={TRANSFER_LR}, fast_lr={FAST_LR}")
    print(f"  v2 params: auto_granularity={AUTO_GRANULARITY}, "
          f"in_chop_prob={IN_CHOP_PROB}, auto_momentum={AUTO_MOMENTUM}")
...
def main():
    """Train MobileBERT with TikiTaka v2 on SQuAD."""
    manual_seed(SEED)
    set_seed(SEED)
    if USE_CUDA:
    ...
    wandb.init(
        ...
        name=f"mobilebert_tiki2_te{TRANSFER_EVERY}_bs{BATCH_SIZE}",
        config={
            ...
            "device_type": "tikitaka_v2_chopped",
        }
    )
