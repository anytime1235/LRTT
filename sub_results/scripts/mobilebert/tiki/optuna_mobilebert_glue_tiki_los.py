...
GRID_TE = [32, 64, 128]
GRID_FLR = [0.001, 0.005, 0.01]

# TPE search ranges (used when --sampler tpe)
TPE_LR_RANGE = (0.001, 0.1)         # log-uniform
TPE_TE_CHOICES = [32, 64, 128]      # categorical (task-specific, overridden below)
TPE_FLR_RANGE = (0.001, 0.01)       # log-uniform
SAMPLER_TYPE = "grid"                # "grid" or "tpe"

# Task-specific te choices (uim=False, 32-unit increments, need ≥3 transfers in 5 epochs)
# RTE:195, MRPC:290, STS-B:450, CoLA:670, SST-2:5265, QNLI:8185, QQP:28430, MNLI:30680
TASK_TO_TE_CHOICES = {
    "rte":  [32, 64],
    "mrpc": [32, 64, 96],
    "stsb": [32, 64, 96, 128],
    "cola": [32, 64, 96, 128, 160, 192, 224],
    "sst2": [32, 64, 96, 128, 160, 192, 224, 256],
    "qnli": [32, 64, 96, 128, 160, 192, 224, 256],
    "qqp":  [32, 64, 96, 128, 160, 192, 224, 256],
    "mnli": [32, 64, 96, 128, 160, 192, 224, 256],
}
...
def set_fast_lr_all_tiles(model, fast_lr):
    """Update fast_lr on all TikiTaka tiles at runtime."""
    from aihwkit.nn import AnalogLinear
    for m in model.modules():
        if isinstance(m, AnalogLinear):
            for tile in m.analog_tiles():
...
def objective(trial, train_loader, eval_loader):
    ...
    # Task-specific te choices
    te_choices = TASK_TO_TE_CHOICES.get(TASK_NAME, [32, 64, 128])
    ...
    if SAMPLER_TYPE == "tpe":
        ...
        transfer_every = trial.suggest_categorical('transfer_every', [t for t in GRID_TE if t in te_choices] or te_choices)
    ...
    try:
        ...
        for epoch in range(1, N_EPOCHS + 1):
            ...
            for batch in pbar:
                ...
                # f_lr scheduling: linear decay matching lr schedule
                flr_decay = max(0.0, 1.0 - global_step / num_training_steps)
                current_flr = fast_lr * flr_decay
                set_fast_lr_all_tiles(model, current_flr)
                ...
                total_loss += loss_val
                num_batches += 1
                pbar.set_postfix(loss=f"{loss_val:.4f}", flr=f"{current_flr:.5f}")
            ...
            if epochs_without_improvement >= 2:
                tqdm.write(f"[Trial {trial.number}] Early stopping at epoch {epoch} (no improvement for 2 epochs)")
                break
...
def main():
    global BATCH_SIZE, N_EPOCHS, WARMUP_RATIO, LORA_TARGET, HEAD_LAYER, TASK_NAME
    ...
    if SAMPLER_TYPE == "tpe":
        ...
        print(f"Sampler: TPE | lr={TPE_LR_RANGE} | te={te_choices} | flr={TPE_FLR_RANGE}")
    else:
        GRID_LR = args.fix_lr if args.fix_lr else [0.001, 0.01, 0.1]
        GRID_TE = args.fix_te if args.fix_te else te_choices
        GRID_FLR = args.fix_flr if args.fix_flr else [0.001, 0.005, 0.01]
        grid_search_space = {
