# -*- coding: utf-8 -*-
"""Optuna hyperparameter sweep for MobileBERT + SQuAD with TikiTaka v2.

Usage:
    python optuna_mobilebert_squad_tiki2.py --n-trials 50
    python optuna_mobilebert_squad_tiki2.py --visualize
    python optuna_mobilebert_squad_tiki2.py --n-trials 50 --optimizer AnalogSGD --no-wd --no-momentum --no-nesterov --batch-size 64 --epochs 15 --warmup-steps 500 --lora-target qkv

All flags:
    python optuna_mobilebert_squad_tiki2.py \
...
DEFAULT_STUDY_NAME = "mobilebert_squad_tiki2_main"
...
# Paths
RESULTS = "/data/results/tikitakav2"
...
# Scheduler
WARMUP_STEPS = 500
WARMUP_RATIO = None  # If set, overrides WARMUP_STEPS (e.g. 0.05 = 5%)
...
def create_tikitaka_v2_config(transfer_every, transfer_lr, fast_lr,
    ...
    rpu_config = UnitCellRPUConfig(
        device=ChoppedTransferCompound(
            ...
            units_in_mbatch=False,     # mat-vec units (finer transfer control)
...
def create_model(params):
    ...
    Architecture:
        - Target layers (based on --lora-target) -> TikiTaka v2 Analog
    ...
    print(f"  TikiTaka v2 Analog layers: {analog_count}, Total params: {total_params:,}, Trainable (before grad set): {trainable_before:,}")
...
def objective(trial, train_loader, eval_features, eval_examples, tokenizer):
    ...
    # Grid search parameters (TTv2)
    learning_rate = trial.suggest_categorical('learning_rate', [0.1])
    transfer_every = trial.suggest_categorical('transfer_every', [1, 8, 16, 32])
    fast_lr = trial.suggest_categorical('fast_lr', [0.1, 5.0])
    transfer_lr = trial.suggest_categorical('transfer_lr', [1.0])
    auto_granularity = 500.0  # fixed
    in_chop_prob = 0.1  # fixed
    min_lr_rate = 0.0
    ...
    try:
        ...
        if LORA_TARGET == "none":
            ...
            # TikiTaka modes: use Analog optimizers
            if optimizer_name == "AnalogSGD":
                optimizer = AnalogSGD(
                    model.parameters(), lr=learning_rate,
                    weight_decay=weight_decay, momentum=momentum, nesterov=nesterov,
        ...
        for epoch in range(1, N_EPOCHS + 1):
            ...
            if epoch <= 3 and best_f1 < 20.0:
                if epoch == 3:
                    tqdm.write(f"[Trial {trial.number}] Early stopping: F1 {best_f1:.2f}% < 20% at epoch 3")
                    break
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Optuna sweep for MobileBERT SQuAD TikiTaka v2")
    ...
    study_name = args.study_name or f"mobilebert_squad_tiki2_bs{BATCH_SIZE}_{get_study_name_suffix()}"
