# -*- coding: utf-8 -*-
"""MobileBERT + GLUE with TikiTaka v2.

Single-run training script for MobileBERT on GLUE tasks using TikiTaka v2 analog layers.
Converts target attention/FFN layers to analog; all other layers remain digital.

Based on mobilebert_glue_tiki.py, converted from TikiTaka v1 to TikiTaka v2
...
Inline flags (edit directly in script):
    ...
    TRANSFER_EVERY = 1               # Transfer interval (mat-vec units)
    TRANSFER_LR = 1.0                # Transfer learning rate
    FAST_LR = 1.0                    # Fast tile learning rate
    AUTO_GRANULARITY = 0.0           # Buffer auto-granularity
...
# Paths
RESULTS = os.path.join(os.getcwd(), "results", "MOBILEBERT_GLUE_TIKI2")
...
# WandB
WANDB_PROJECT = "mobilebert-glue-tiki2"
...
def create_model():
    """Create MobileBERT classification model with selective TikiTaka v2 analog layers.
    ...
    if LORA_TARGET == "none":
        ...
        tiki_config = create_tikitaka_v2_config()
    ...
    print(f"\nCreated MobileBERT model (TikiTaka v2, GLUE):")
    ...
    print(f"  TikiTaka v2 config: transfer_every={TRANSFER_EVERY}, "
          ...
          f"auto_granularity={AUTO_GRANULARITY}, in_chop_prob={IN_CHOP_PROB}")
...
def main():
    """Train MobileBERT with TikiTaka v2 on GLUE."""
    manual_seed(SEED)
    set_seed(SEED)
    if USE_CUDA:
    ...
    weight_path = os.path.join(RESULTS, f"mobilebert_glue_tiki2_{TASK_NAME}_model_weight.pth")

    wandb.init(
        project=WANDB_PROJECT,
        name=f"mobilebert_tiki2_{TASK_NAME}_te{TRANSFER_EVERY}_bs{BATCH_SIZE}",
        config={
