# -*- coding: utf-8 -*-
"""Optuna hyperparameter sweep for MobileBERT + GLUE with TikiTaka v2.

Usage:
    python optuna_mobilebert_glue_tiki2.py --task sst2 --n-trials 50
    python optuna_mobilebert_glue_tiki2.py --task sst2 --visualize
    python optuna_mobilebert_glue_tiki2.py --task cola --n-trials 50 --optimizer AnalogSGD --no-wd --no-momentum --no-nesterov

All flags:
    python optuna_mobilebert_glue_tiki2.py \
...
TASK_TO_LR = {
    "rte": 6e-3, "mrpc": 6e-3, "stsb": 1e-3, "cola": 1e-3,
    "sst2": 1e-3, "qnli": 1e-3, "qqp": 1e-3, "mnli": 1e-3,
}
...
# Paths
RESULTS = "/data/results/tikitakav2_glue"
...
TPE_FLR_RANGE = (0.01, 1.0)         # log-uniform (aihwkit default=1.0)
TPE_TLR_RANGE = (0.01, 1.0)         # log-uniform (aihwkit default=1.0)
...
def create_tikitaka_v2_config(transfer_every, transfer_lr, fast_lr,
                               auto_granularity=0.0, in_chop_prob=0.1):
    ...
    rpu_config.backward.is_perfect = False
...
def create_single_rpu_config():
    ...
    Uses the same SoftBoundsDevice as TikiTaka's C tile (slow, accurate).
    Tile weights are frozen via noop update hook (see create_model).
    """
    c_device = _create_c_device()

    rpu_config = SingleRPUConfig(device=c_device)
...
def create_model(params):
    ...
    def is_tikitaka_target(layer_name):
        """Check if encoder layer should be TikiTaka v2 (trainable analog)."""
    ...
    # --- Pass 1: Convert target layers to TikiTaka v2 ---
    tikitaka_count = 0
    if tikitaka_layers and LORA_TARGET != "none":
        tiki2_config = create_tikitaka_v2_config(
            ...
            auto_granularity=params.get("auto_granularity", 0.0),
            in_chop_prob=params.get("in_chop_prob", 0.1),
        )
        tiki_exclude = [n for n in all_linear_names if n not in tikitaka_layers]
        model = convert_to_analog(model, tiki2_config, exclude_modules=tiki_exclude)
    ...
    print(f"  TikiTaka v2: {tikitaka_count}, NT Analog: {single_rpu_count}, "
...
def set_fast_lr_all_tiles(model, fast_lr):
    """Update fast_lr on all TikiTaka v2 tiles at runtime."""
    from aihwkit.nn import AnalogLinear
    for m in model.modules():
        if isinstance(m, AnalogLinear):
            for tile in m.analog_tiles():
                if isinstance(tile.rpu_config, UnitCellRPUConfig):
                    tile.rpu_config.device.fast_lr = fast_lr
...
def objective(trial, train_loader, eval_loader):
    ...
    # Learning rate: fixed per task (not swept)
    learning_rate = TASK_TO_LR.get(TASK_NAME, 0.001)

    # fast_lr, transfer_lr: TTv2 preset defaults
    fast_lr = 0.5
    transfer_lr = 1.0

    # transfer_every: fixed at 1 (uim=False, mat-vec units, C++ handles multi-transfer/batch)
    transfer_every = 1

    # v2 specific: TTv2 preset defaults
    ...
    print(f"  auto_granularity={auto_granularity}, in_chop_prob={in_chop_prob}")
    ...
    try:
        ...
        for epoch in range(1, N_EPOCHS + 1):
            ...
            for batch in pbar:
                ...
                # fast_lr: constant (no decay)
                set_fast_lr_all_tiles(model, fast_lr)
                ...
                # Loss divergence detection: NaN or extremely large
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Optuna sweep for MobileBERT GLUE TikiTaka v2")
    ...
    parser.add_argument('--tpe-flr-range', type=float, nargs=2, default=None,
                        help='TPE fast_lr range (default: 0.01 1.0)')
    ...
    study_name = args.study_name or f"mobilebert_tiki2_{TASK_NAME}_{timestamp}"
    ...
    print(f"  transfer_every=1 (fixed, uim=False, mat-vec units, TTv2 preset)")
    print(f"  fast_lr=0.5 (fixed), transfer_lr=1.0 (fixed), auto_granularity=1000, in_chop_prob=0.0")
