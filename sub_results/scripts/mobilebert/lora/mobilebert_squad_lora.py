# -*- coding: utf-8 -*-
"""MobileBERT + SQuAD with LoRA-LRTT (LoRA mode using LRTT analog devices).

Single-run training script for MobileBERT on SQuAD using LoRA-style forward:
  y = C·x + α·A·(B·x)
...
Based on mobilebert_squad_lrtt_scratch.py.
...
LORA_ALPHA = 0.0321  # LoRA alpha scaling (forward: y = C·x + α·A·(B·x))
LRTT_LR_MULTIPLIER = 0.005309  # LRTT lr = LEARNING_RATE * LRTT_LR_MULTIPLIER (target_ab_lr=0.002678)
REINIT_MODE = "decay"
REINIT_GAIN = 1.0
DECAY_FACTOR = 1.0
...
NORM_TYPE = "no_norm"  # "layer_norm" (default) or "no_norm"
...
def create_lrtt_config():
    ...
    # Combined out_scaling: shared learnable scale for full LRTT output
...
def create_model():
    ...
    for name, param in model.named_parameters():
        if "tile_a" in name or "tile_b" in name:
            ...
            # LRTT tile_c individual out_scaling: FROZEN
    ...
    # The freeze loop above sets all non-LRTT params to requires_grad=False,
    ...
    # Without this, backward stores inputs in analog_ctx.analog_input every step
    # but reset() is never called → unbounded memory growth → OOM.
    if CONVERT_NONTARGET and LORA_TARGET != "none":
        nt_ctx_count = 0
        nt_scale_count = 0
        for name, param in model.named_parameters():
            if 'analog_ctx' in name and not any(t in name for t in ['tile_a', 'tile_b', 'tile_c']):
                param.requires_grad = True
                nt_ctx_count += 1
            # NT out_scaling_alpha: trainable for output scale adjustment
            if 'out_scaling_alpha' in name and not any(t in name for t in ['tile_a', 'tile_b', 'tile_c']):
                param.requires_grad = True
                nt_scale_count += 1
        print(f"  Re-enabled requires_grad for {nt_ctx_count} NT analog_ctx params")
        print(f"  Re-enabled requires_grad for {nt_scale_count} NT out_scaling_alpha params")
...
def main():
    ...
    def _mem(tag=""):
        if USE_CUDA:
            ...
            print(f"  [GPU-MEM] {tag}: alloc={alloc:.3f}GB  reserved={resv:.3f}GB  peak={peak:.3f}GB")

    _mem("after-model+optimizer")
