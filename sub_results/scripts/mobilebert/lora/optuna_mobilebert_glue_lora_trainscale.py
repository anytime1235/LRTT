...
# Training defaults
N_EPOCHS = 3  # GLUE standard
BATCH_SIZE = 64
EVAL_BATCH_SIZE = 64  # GLUE standard
EARLY_STOP_PATIENCE = 3

# Scheduler
WARMUP_RATIO = 0.1  # GLUE: 10% warmup
WARM_ALPHA = False
...
HEAD_LAYER = "train"  # default, can be set via --head-layer (train | freeze) - classifier for SQuAD
LORA_TARGET_MODULES = {
...
def create_model(params):
    ...
    for name, param in model.named_parameters():
        if "tile_a" in name or "tile_b" in name:
            ...
            # LRTT out_scaling: TRAINABLE (compensates for analog DAC/ADC distortion)
            param.requires_grad = True
...
def objective(trial, train_loader, eval_loader, tokenizer):
    ...
    if OPT_CONFIG['no_transfer']:
        ...
        # LoRA mode: no transfer, forward_inject=True, tune lora_alpha
        transfer_lr = 0.1  # Fixed (not used in LoRA mode)
        transfer_every = 10000000  # Fixed (transfer disabled)
        rank = 8  # Fixed
        # TPE search: lora_alpha 0.0001~0.01, ab_lr from [1.0, 0.1, 0.01]
        lora_alpha = trial.suggest_float('lora_alpha', 1e-4, 1e-2, log=True)
        ab_lr = trial.suggest_categorical('ab_lr', [1.0, 0.1, 0.01])
        lrtt_lr_multiplier = ab_lr / (learning_rate * lora_alpha)  # Effective A,B tile lr = ab_lr
    ...
    try:
        ...
        if LORA_TARGET == "none":
            ...
            other_params = []      # classifier (classifier) and other trainable parameters
        ...
        for epoch in range(1, N_EPOCHS + 1):
            ...
            for batch_idx, batch in enumerate(pbar):
                ...
                # FORWARD_INJECT FIX: Reset all tile contexts not in optimizer
                # tile_c: always not in optimizer (requires_grad=False)
                # tile_a/b: not in optimizer when --no-transfer (requires_grad=False)
                # Without reset, analog_input/analog_grad_output lists grow unboundedly
                if LORA_TARGET != "none":
...
def main():
    global TASK_NAME, BATCH_SIZE, N_EPOCHS, WARMUP_RATIO, WARM_ALPHA, TRANSFER_METHOD, AB_DEVICE, IO_NOISE, LORA_TARGET, HEAD_LAYER, RESULTS
    ...
    TASK_NAME = args.task
    RESULTS = f"/data/results/Analoglora_v2_trainscale/{TASK_NAME}"
    ...
    # TPESampler for continuous lora_alpha exploration
