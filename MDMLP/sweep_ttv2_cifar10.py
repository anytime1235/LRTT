#!/usr/bin/env python3
"""Optuna-based hyperparameter sweep for MDMLP + TikiTaka v2 on CIFAR-10.

TikiTaka v2 uses ChoppedTransferCompound with 6T1C Fast + SoftBounds Slow tiles
(same devices as LRTT). Key v2 features: chopped updates, auto scaling.

Searches over: lr, transfer_lr, transfer_every, fast_lr, in_chop_prob,
               auto_granularity, weight_decay.

Usage:
    python sweep_ttv2_cifar10.py --n-trials 50 --epochs 50
    python sweep_ttv2_cifar10.py --n-trials 100 --epochs 200 --study-name full_sweep
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Optuna sweep for MDMLP + TikiTaka v2")
    ...
    parser.add_argument("--study-name", type=str, default="mdmlp_ttv2_cifar10",
                        help="Optuna study name")
    parser.add_argument("--storage", type=str, default=None,
                        help="Optuna storage URL (e.g., sqlite:///sweep.db). None=in-memory")
    parser.add_argument("--output-dir", type=str, default="./output/sweep_ttv2",
...
def objective(trial, args):
    """Optuna objective: run one MDMLP+TTv2 training and return best val accuracy."""
    ...
    cmd = [
        ...
        args.data_dir,
        "-c", os.path.join(os.path.dirname(__file__), "ymls", "cifar10_ttv2.yml"),
        "--model", args.model,
        "--analog",
        "--algo", "ttv2",
...
def main():
    ...
    print("\n" + "=" * 80)
    print("TIKITAKA v2 (TTv2) SWEEP COMPLETE")
    print("=" * 80)
