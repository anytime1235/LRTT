#!/usr/bin/env python3
"""Optuna sweep for MDMLP + LRTT with hybrid reinit, 200ep cosine annealing.

Grid over transfer_every: [50, 100, 500, 1000]
Search lr and transfer_lr with Optuna per each te value.
reinit_mode=hybrid, stem/head=FloatingPoint, cosine scheduler, early stopping.
Matches TikiTaka configuration: 48 analog layers + 18 digital (stem+head+clayer).

Usage:
    python sweep_lrtt_hybrid_200ep.py --n-trials 40 --epochs 200 --patience 10
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Optuna sweep: LRTT hybrid reinit, 200ep cosine")
    ...
    parser.add_argument("--output-dir", type=str, default="/root/results/sweep_lrtt_hybrid_200ep")
...
def run_trial(trial, args, transfer_every):
    ...
    print(f"reinit=hybrid, decay_factor={args.decay_factor}, stem/head=FloatingPoint,"
...
def main():
    ...
    for te in te_values:
        ...
        study_name = f"lrtt_hybrid_200ep_te{te}"
        ...
        print(f"    (reinit=hybrid, stem/head=FloatingPoint, 48 analog layers)")
