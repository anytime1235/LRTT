#!/usr/bin/env python
# coding=utf-8
"""Test TikiTaka v2 with mapping config on SQuAD using best params from Trial 24.

Compares performance WITH vs WITHOUT mapping (weight_scaling + learn_out_scaling).
Runs 3 epochs with the best hyperparameters found in the sweep.
"""
...
# ============================================================
# Best params from SQuAD sweep Trial 24 (F1=60.95)
# ============================================================
BEST_PARAMS = {
    "learning_rate": 0.00019309457791207612,
    "transfer_lr": 1.8315021138603822,
    "transfer_every": 63,
    "fast_lr": 0.2628870678631465,
    "auto_granularity": 483.07396584625275,
    "in_chop_prob": 0.045133733242535556,
}
...
# Config creation
# ============================================================
def create_config(use_mapping: bool) -> UnitCellRPUConfig:
...
def run_experiment(use_mapping: bool, device):
    label = "WITH mapping" if use_mapping else "WITHOUT mapping (baseline)"
    print(f"\n{'='*60}")
    print(f"  Running SQuAD experiment: {label}")
    print(f"{'='*60}")
    ...
    rpu_config = create_config(use_mapping)
    print(f"Mapping config: omega={rpu_config.mapping.weight_scaling_omega}, "
          f"learn_out_scaling={rpu_config.mapping.learn_out_scaling}, "
          f"columnwise={rpu_config.mapping.weight_scaling_columnwise}")
    ...
    results_per_epoch = [{"epoch": 0, "f1": init_f1, "em": init_em}]
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        results_per_epoch.append({"epoch": epoch, "loss": avg_loss, "f1": f1, "em": em})
...
def main():
    ...
    print(f"Best params (Trial 24): {BEST_PARAMS}")

    # Run WITHOUT mapping (baseline - same as original sweep)
    results_without = run_experiment(use_mapping=False, device=device)

    # Run WITH mapping (new)
    results_with = run_experiment(use_mapping=True, device=device)

    # Compare
    print("\n" + "="*60)
    print("  COMPARISON: WITHOUT vs WITH mapping")
    print("="*60)
    print(f"{'Epoch':<8} {'F1 (no map)':<15} {'F1 (mapped)':<15} {'Delta':<10}")
    print("-"*48)
    for r_no, r_yes in zip(results_without, results_with):
        delta = r_yes["f1"] - r_no["f1"]
        sign = "+" if delta > 0 else ""
        print(f"{r_no['epoch']:<8} {r_no['f1']:<15.2f} {r_yes['f1']:<15.2f} {sign}{delta:<10.2f}")

    final_no = results_without[-1]["f1"]
    final_yes = results_with[-1]["f1"]
    print(f"\nFinal F1 WITHOUT mapping: {final_no:.2f}")
    print(f"Final F1 WITH mapping:    {final_yes:.2f}")
    print(f"Improvement:              {final_yes - final_no:+.2f}")

    # Save results
    output = {
        "best_params": BEST_PARAMS,
        "without_mapping": results_without,
        "with_mapping": results_with,
        "improvement": final_yes - final_no,
    }
    output_path = "/data/AIMC_LoRA_results/tikitaka_sweep/remap_test_squad_results.json"
