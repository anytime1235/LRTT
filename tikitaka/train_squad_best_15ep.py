#!/usr/bin/env python
# coding=utf-8
"""Train TikiTaka v2 on SQuAD with best params from sweep Trial 42 (F1=61.93).

Runs 15 epochs with mapping (weight_scaling + learn_out_scaling) enabled.
"""
...
def main():
    ...
    # Initialize wandb
    run = wandb.init(
        project="tikitaka-v2-squad-15ep",
        name=f"squad_trial42_best_15ep",
        config={
            "task": "squad",
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        if f1 > best_f1:
            ...
            wandb.log({"best/f1": best_f1, "best/epoch": epoch})

    # Summary
    print(f"\n{'='*60}")
    print(f"  TRAINING COMPLETE - 15 Epochs")
    print(f"{'='*60}")
    print(f"Best F1: {best_f1:.2f}")
    for r in results_per_epoch:
        marker = " *" if r["f1"] == best_f1 and r["epoch"] > 0 else ""
        loss_str = f", Loss: {r['loss']:.4f}" if "loss" in r else ""
        print(f"  Epoch {r['epoch']:>2d}: F1={r['f1']:.2f}, EM={r['em']:.2f}{loss_str}{marker}")
    ...
    output_path = "/data/AIMC_LoRA_results/tikitaka_sweep/squad_trial42_best_15ep_results.json"
