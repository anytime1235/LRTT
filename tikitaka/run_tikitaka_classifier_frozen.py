...
def create_glue_model(task_name: str, params: Dict, device: torch.device) -> nn.Module:
    ...
    for name, param in model.named_parameters():
        ...
        if "bias" in name:
            ...
            param.requires_grad = False  # FREEZE classifier too
        else:
...
def run_trial(trial: optuna.Trial, task_name: str, train_loader, eval_loader,
    ...
    try:
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            # Report to Optuna for potential pruning (same as sixt1c)
...
def main():
    """Single trial with best params from sst2_sweep_batch256_bias_frozen.json + classifier FROZEN."""

    # Best params from sweep (Trial 33, acc=90.14%)
    BEST_PARAMS = {
        "learning_rate": 0.0023065114558800644,
        "transfer_lr": 3.986266963830462,
        "fast_lr": FAST_LR_FIXED,  # 3.0
        "transfer_every": 10000000,  # 10^7: C tile never updated
        "auto_granularity": FIXED_AUTO_GRANULARITY,  # 169.0
        "in_chop_prob": FIXED_IN_CHOP_PROB,  # 0.061
    }

    task_name = "sst2"
    num_epochs = GLUE_EPOCHS  # 3

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    results_dir = os.path.join(OUTPUT_DIR, f"classifier_frozen_te1e7_{timestamp}")
    os.makedirs(results_dir, exist_ok=True)

    print("=" * 60)
    print("TikiTaka v2 - Single Trial (Classifier FROZEN)")
    print("=" * 60)
    ...
    # Create model (classifier frozen via modified create_glue_model)
    model = create_glue_model(task_name, BEST_PARAMS, device)
    ...
    print(f"\n[Before training] Accuracy: {init_metric:.4f}, Loss: {init_loss:.4f}")
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        print(f"[Epoch {epoch}/{num_epochs}] Train loss: {train_loss:.4f}, "
              f"Accuracy: {eval_metric:.4f}, Best: {best_metric:.4f}")

    # Save result
    result = {
        "experiment": "classifier_frozen_single_trial",
        "task": task_name,
    ...
    result_file = os.path.join(results_dir, f"{task_name}_classifier_frozen_result.json")
    with open(result_file, 'w') as f:
        json.dump(result, f, indent=2)

    print(f"\n{'='*60}")
    print(f"DONE - Best accuracy: {best_metric:.4f}")
