...
def main():
    """Single trial baseline with AnalogSGD: classifier trainable, te=491."""
    ...
    task_name = "sst2"
    num_epochs = GLUE_EPOCHS

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    results_dir = os.path.join(OUTPUT_DIR, f"sgd_baseline_{timestamp}")
    os.makedirs(results_dir, exist_ok=True)

    print("=" * 60)
    print("TikiTaka v2 - AnalogSGD Baseline (Classifier TRAINABLE)")
    print("=" * 60)
    ...
    optimizer = AnalogSGD(model.parameters(), lr=BEST_PARAMS["learning_rate"])
    optimizer.regroup_param_groups(model)
    ...
    result = {
        "experiment": "sgd_baseline_classifier_trainable",
        "task": task_name,
        "optimizer": "AnalogSGD",
    ...
    result_file = os.path.join(results_dir, f"{task_name}_sgd_baseline_result.json")
