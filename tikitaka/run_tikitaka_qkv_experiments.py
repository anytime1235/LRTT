...
Key Changes from previous TikiTaka v2:
- **Fast tile (A): Changed from 6T1C to SoftBounds (noise=0)**
- Slow tile (C): SoftBounds (noise=0) - unchanged
...
EXPERIMENTS = [
    {"name": "Q_only", "target_modules": ["query"]},
    {"name": "K_only", "target_modules": ["key"]},
    {"name": "V_only", "target_modules": ["value"]},
    {"name": "QKV_all", "target_modules": ["query", "key", "value"]},
]
...
def create_tikitaka_softbounds_config(
    ...
    Key change: Fast tile is now SoftBounds instead of 6T1C LinearStepDevice.
    Both tiles use idealized (noise-free) SoftBoundsReferenceDevice.
    """

    # Fast Tile (A): SoftBounds (noise=0) - CHANGED from 6T1C
