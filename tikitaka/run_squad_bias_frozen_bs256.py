#!/usr/bin/env python
"""SQuAD TikiTaka v2 — Best Config with Bias Frozen (bs256, 15ep).

Runs the best config from sweep_squad_15ep_bs256.py (Trial 44, F1=74.42)
but with ALL bias parameters frozen, matching lora_on_analog_hardware setup.

Purpose: Fair comparison — previous sweep had bias trainable.
"""
...
# Best params from Trial 44 (F1=74.42, bias was trainable)
BEST_PARAMS = {
    "learning_rate": 0.0012609,
    "transfer_lr": 0.050069,
    "fast_lr": 2.192782,
    "transfer_every": 10000000,
...
def create_tikitaka_v2_config(transfer_every, transfer_lr, fast_lr,
    ...
    rpu_config = UnitCellRPUConfig(
        device=ChoppedTransferCompound(
            ...
            transfer_every=transfer_every, units_in_mbatch=False,
            n_reads_per_transfer=1, transfer_columns=True, gamma=0.0,
            transfer_lr=transfer_lr, fast_lr=fast_lr, scale_transfer_lr=True,
            auto_scale=True, auto_granularity=auto_granularity,
            buffer_granularity=1.0, auto_momentum=0.99,
            in_chop_prob=in_chop_prob, in_chop_random=True,
    ...
    # Forward/Backward IO — remove noise, keep ABS_MAX / ITERATIVE
...
# =============================================================================
# Model Creation (with bias freeze + verification)
# =============================================================================
...
def create_squad_model(params, device):
    ...
    # Freeze ALL bias, enable only target weights + qa_outputs + out_scaling
...
def verify_bias_freeze(model):
    """Verify all bias params are frozen. Returns (n_trainable_bias, summary_str)."""
    trainable_bias = []
    frozen_bias = []
    trainable_other = []
    ...
    for name, p in model.named_parameters():
        if "bias" in name:
            (trainable_bias if p.requires_grad else frozen_bias).append((name, p.numel()))
        elif p.requires_grad:
            trainable_other.append((name, p.numel()))

    total_params = sum(p.numel() for p in model.parameters())
    trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    total_bias = sum(n for _, n in trainable_bias) + sum(n for _, n in frozen_bias)

    lines = []
    lines.append(f"  Total params:      {total_params:,}")
    lines.append(f"  Trainable params:  {trainable_params:,} ({100*trainable_params/total_params:.2f}%)")
    lines.append(f"  Total bias params: {total_bias:,}")
    lines.append(f"  Trainable bias:    {sum(n for _, n in trainable_bias):,}")
    lines.append(f"  Frozen bias:       {sum(n for _, n in frozen_bias):,} ({len(frozen_bias)} tensors)")
    lines.append(f"  Trainable non-bias: {len(trainable_other)} tensors")

    if trainable_bias:
        lines.append(f"  [FAIL] Trainable biases found:")
        for name, numel in trainable_bias:
            lines.append(f"    TRAINABLE: {name} ({numel})")
    else:
        lines.append(f"  [PASS] All biases frozen.")

    summary = "\n".join(lines)
    return len(trainable_bias), summary
...
def evaluate_squad(model, eval_features, eval_examples, tokenizer, device):
    ...
    def collate_fn(features):
        ...
        eids = [f.pop("example_id") for f in features]
        batch = default_data_collator(features)
        for i, f in enumerate(features):
            f["offset_mapping"] = offsets[i]
            f["example_id"] = eids[i]
        return batch
    ...
    predictions = postprocess_squad_predictions(
        eval_examples, eval_features, all_start_logits, all_end_logits)
...
def main():
    ...
    results_dir = os.path.join(OUTPUT_DIR, f"squad_bias_frozen_{timestamp}")
    os.makedirs(results_dir, exist_ok=True)

    print("=" * 60)
    print("SQuAD TikiTaka v2 — Best Config with Bias Frozen")
    print("=" * 60)
    print(f"Baseline: Trial 44, F1=74.42 (bias trainable)")
    print(f"This run: same config, ALL bias frozen")
    ...
    # Create model
    print("\nCreating model...")
    model = create_squad_model(BEST_PARAMS, device)

    # Verify bias freeze
    n_bad, summary = verify_bias_freeze(model)
    print(f"\nBias Freeze Verification:")
    print(summary)
    if n_bad > 0:
        print("ABORTING: bias not properly frozen!")
        return
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        for batch in pbar:
            ...
            if math.isnan(loss_val) or math.isinf(loss_val) or loss_val > LOSS_EXPLOSION_THRESHOLD:
                print(f"  Loss diverged ({loss_val}) at epoch {epoch}!")
                diverged = True
                break
        ...
        if diverged:
            print("Training stopped due to loss divergence.")
            break
        ...
        results_log.append({
            ...
            "f1": eval_f1, "em": eval_em, "lr": current_lr,
        })

        print(f"  [Epoch {epoch}/{NUM_EPOCHS}] Loss: {avg_loss:.4f}, F1: {eval_f1:.2f}, EM: {eval_em:.2f}, LR: {current_lr:.6f}")
        ...
        if patience_counter >= PATIENCE:
            print(f"  Early stopping at epoch {epoch} (best F1={best_f1:.2f})")
            break

    # Save results
    result = {
        "experiment": "tikitaka_v2_bias_frozen",
        "baseline_f1": 74.42,
        "best_f1": best_f1,
        "delta": best_f1 - 74.42,
        "params": BEST_PARAMS,
        "bias_frozen": True,
        "epochs_log": results_log,
    }
    ...
    print(f"  RESULT")
    print(f"{'='*60}")
    print(f"  Baseline (bias trainable):  F1 = 74.42")
    print(f"  This run (bias frozen):     F1 = {best_f1:.2f}  ({best_f1 - 74.42:+.2f})")
