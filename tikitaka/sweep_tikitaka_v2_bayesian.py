#!/usr/bin/env python
# coding=utf-8
"""TikiTaka v2 (ChoppedTransferCompound) Bayesian Optimization Sweep with WandB Logging.
...
- transfer_lr: [0.1, 2.0] (linear scale)
- transfer_every: [1, 100] (log scale, integer) - mat-vec units
- fast_lr: [0.1, 1.0] (linear scale)
- auto_granularity: [100, 5000] (log scale)
- in_chop_prob: [0.0, 0.1] (linear scale)

Fixed settings:
- rank: 8 (not used, query only)
...
TRANSFER_LR_MIN, TRANSFER_LR_MAX = 0.1, 2.0
TRANSFER_EVERY_MIN, TRANSFER_EVERY_MAX = 1, 100  # mat-vec units (v2)

# TikiTaka v2 specific parameters
FAST_LR_MIN, FAST_LR_MAX = 0.1, 1.0
AUTO_GRAN_MIN, AUTO_GRAN_MAX = 100, 5000
CHOP_PROB_MIN, CHOP_PROB_MAX = 0.0, 0.1
...
# WandB settings
WANDB_PROJECT = "tikitaka-v2-sweep"
WANDB_ENTITY = None
...
def main():
    ...
    print("=" * 80)
    print("TikiTaka v2 (ChoppedTransferCompound) Bayesian Optimization Sweep with WandB")
    ...
    print(f"  learning_rate: [{LR_MIN:.0e}, {LR_MAX:.0e}] (log)")
    print(f"  transfer_lr: [{TRANSFER_LR_MIN}, {TRANSFER_LR_MAX}] (linear)")
    print(f"  transfer_every: [{TRANSFER_EVERY_MIN}, {TRANSFER_EVERY_MAX}] (log)")
    print(f"  fast_lr: [{FAST_LR_MIN}, {FAST_LR_MAX}] (linear)")
    print(f"  auto_granularity: [{AUTO_GRAN_MIN}, {AUTO_GRAN_MAX}] (log)")
    print(f"  in_chop_prob: [{CHOP_PROB_MIN}, {CHOP_PROB_MAX}] (linear)")
    ...
    # CSV logger
    csv_path = os.path.join(OUTPUT_DIR, f'tikitaka_v2_bayesian_results_{timestamp}.csv')
    ...
    study = optuna.create_study(study_name=f"tikitaka_v2_{timestamp}", direction="maximize", sampler=sampler)
    ...
    # Save JSON summary
    json_path = os.path.join(OUTPUT_DIR, f'tikitaka_v2_bayesian_summary_{timestamp}.json')
