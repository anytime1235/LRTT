...
# TikiTaka (2-tile: Fast 6T1C + Slow SoftBounds) parameters
TIKITAKA_PARAMS: Dict[str, Any] = {
    # Transfer settings
    "transfer_every": 32,          # Transfer frequency in mini-batches
    "transfer_lr": 1.0,            # Learning rate for transfer
    "fast_lr": 1.0,                # Fast tile learning rate multiplier
    "gamma": 0.0,                  # Only Slow/C tile visible (0=Slow, 1=Fast)

    # 6T1C Fast tile params (match SIXT1C for consistency)
    "dw_min": 0.001981,
    "gamma_up": -0.1678,
    "gamma_down": 0.1410,
    "write_noise_std": 0.0,        # Write noise (set to 0 as requested)
    ...
    # Slow tile
    "slow_noise_free": True,       # Noise-free SoftBounds for stable storage

    # Optimizer (same as sixt1c)
    "analog_optimizer": "AnalogAdam",
    "analog_lr": 0.0002,
    "num_evaluation_drift_values": 7,
    "num_evaluation_repetition": 10,
