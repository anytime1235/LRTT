"""paper_figures.py — 논문용 Figure A/B/C/D 생성 (v3: AIHWKit-consistent metrics)

Four publication-quality figures based on diag_kv_rootcause.py patterns,
...
- Figure D: Layerwise Mixed-Precision Validation (2×2, selective upgrade)
...
parser = argparse.ArgumentParser(description="Paper Figures A/B/C/D — AIMC BERT-base")
...
parser.add_argument("--layerwise-high-adc-bits", type=int, default=None)
...
# OUT_DIR: prefer /data/main_results/results if it exists, else use args
_default_out = "/data/main_results/results"
OUT_DIR = _default_out if os.path.isdir(_default_out) else args.out_dir
...
print(f"[Config] OUT_DIR={OUT_DIR}, Seeds={SEEDS_LIST}")
print(f"[Config] Figures={args.figures.upper()} (A={RUN_A}, B={RUN_B}, C={RUN_C}, D={RUN_D})")
...
def select_high_modules(model, stats_baseline, policy, args):
    ...
    if policy == "hotspot":
        ...
        if args.layerwise_qzr_threshold is not None:
            selected = df[df["QZR_nonzero"] > args.layerwise_qzr_threshold]
        else:
            selected = df.head(args.layerwise_topk)
        high_pairs = set(zip(selected["layer_idx"], selected["sublayer"]))

    elif policy == "sublayer":
        sub_list = [s.strip() for s in args.layerwise_sublayers.split(",")]
        high_pairs = {(l, s) for (l, s) in pair_to_names.keys() if s in sub_list}

    elif policy == "layer":
        layer_list = [int(x) for x in args.layerwise_layers.split(",") if x.strip()]
...
def create_model_layerwise(high_names, base_dac_bits, base_adc_bits,
    ...
    print(f"  Layerwise analog — high:{n_high}({high_dac_bits}b), "
          f"base:{n_all-n_high}({base_dac_bits}b)")
...
def load_data(tokenizer, n_step, batch_size, seed=42):
...
def run_diagnostic(model, loader, n_step: int, seed: int = 42, desc: str = "Diag"):
...
def figure_A(df: pd.DataFrame, verdict: str, stats_dict: dict,
    ...
    fig.suptitle(
        ...
        f"(BERT-base SQuAD, seed={seed}, {n_step} steps × batch {BATCH_SIZE})\n"
...
def figure_B(df_b: pd.DataFrame, seed: int = 42, output_path: str = ""):
    bits_list_all = sorted(df_b["dac_bits"].unique())

    fig, axes = plt.subplots(2, 2, figsize=(16, 12))
    fig.suptitle(
        f"Figure B — IO Resolution Sweep (BERT-base SQuAD, seed={seed}, "
...
def figure_C(df_c: pd.DataFrame, seed: int = 42, output_path: str = ""):
    ...
    fig.savefig(output_path, dpi=200, bbox_inches="tight")
    plt.close(fig)
    print(f"  Figure C → {output_path}")


# =============================================================================
# Figure D — Layerwise Mixed-Precision Validation (2×2)
# =============================================================================

def figure_D(df_d: pd.DataFrame, high_names: set,
             seed: int = 42, output_path: str = ""):
    """2×2 figure: (a,b) ΔQZR heatmaps, (c,d) grouped bars by sublayer."""
    VARIANTS_D = ["baseline", "full_high", "layerwise"]
    COLORS_D   = ["#4C72B0", "#DD8452", "#55A868"]

    # Build per-variant heatmap matrices (12 layers × 6 sublayers)
    ...
    fig.suptitle(
        ...
        f"baseline={DAC_BITS}b, high={args.layerwise_high_bits}b",
        fontsize=10, y=1.01
    )

    vabs = max(np.nanmax(np.abs(delta_lw)), np.nanmax(np.abs(delta_fh)), 0.01)

    # (a) Heatmap: ΔQZR (layerwise - baseline)
    ax = axes[0, 0]
    ...
    # (b) Heatmap: ΔQZR (full_high - baseline)
    ax = axes[0, 1]
...
def run_single_seed(seed, tokenizer, run_figures):
    """Run full A→B→C→D pipeline for one seed. Returns summary rows."""
    print(f"\n{'='*60}")
    print(f"  SQuAD seed={seed}")
    print(f"{'='*60}")

    seed_dir = os.path.join(OUT_DIR, "squad", f"seed_{seed}")
    os.makedirs(seed_dir, exist_ok=True)

    # Per-seed output paths
    ...
    CSV_C_SUMMARY = os.path.join(seed_dir, "metrics_C_solutions_summary.csv")
    CSV_C_STEPS   = os.path.join(seed_dir, "metrics_C_solutions_steps.csv")
    ...
    FIG_A = os.path.join(seed_dir, "fig_A_rootcause.png")
    FIG_B = os.path.join(seed_dir, "fig_B_bitsweep.png")
    FIG_C = os.path.join(seed_dir, "fig_C_solutions.png")
    FIG_D = os.path.join(seed_dir, "fig_D_layerwise.png")
    ...
    loader = load_data(tokenizer, n_step=n_step_max, batch_size=BATCH_SIZE, seed=seed)

    # ------------------------------------------------------------------ #
    # [2] Baseline run — needed for A, B, C, D                           #
    # ------------------------------------------------------------------ #
    print(f"\n  [2] Baseline 7-bit, {N_STEP} steps ...")
    inp_res_7  = 1.0 / (2**DAC_BITS - 2)
    ...
    save_absmax_npz(stats_baseline,
                    os.path.join(seed_dir, "absmax_raw_A_baseline_7b.npz"))

    # ------------------------------------------------------------------ #
    # [3] Figure A — root cause analysis + figure                        #
    # ------------------------------------------------------------------ #
    if "A" in run_figures:
        if os.path.exists(CSV_A_SUMMARY) and os.path.exists(CSV_A_STEPS):
            print(f"\n  [3] Figure A — SKIP (files exist): {CSV_A_SUMMARY}")
        else:
            print(f"\n  [3] Computing root cause → Figure A ...")
            ...
            figure_A(df_a, verdict_a, stats_baseline, dac_bits=DAC_BITS, n_step=N_STEP,
                     seed=seed, output_path=FIG_A)

    # ------------------------------------------------------------------ #
    # [4] Figure B — bits sweep + baseline 7-bit                         #
    # ------------------------------------------------------------------ #
    if "B" in run_figures:
        if os.path.exists(CSV_B_SUMMARY) and os.path.exists(CSV_B_STEPS):
            print(f"\n  [4] Figure B — SKIP (files exist): {CSV_B_SUMMARY}")
        else:
            print(f"\n  [4] Figure B: bits sweep {[4,6,8,10,12]}, {N_STEP_SWEEP} steps each ...")
            all_b_rows = []
            all_b_step_records = []
            ...
            figure_B(df_b, seed=seed, output_path=FIG_B)

    # ------------------------------------------------------------------ #
    # [5] Figure C — 4 variants                                          #
    # ------------------------------------------------------------------ #
    if "C" in run_figures:
        print(f"\n  [5] Figure C: 4 variants, {N_STEP} steps each ...")
        all_c_rows = []
        all_c_step_records = []
        ...
        stats_p99, handles_p99 = register_hooks(model_p99, mask_buf_p99, inp_res=inp_res_7,
                                                 store_sweep=False)
        run_diagnostic(model_p99, loader, n_step=N_STEP, seed=seed, desc=f"C-p99-s{seed}")
        ...
        save_absmax_npz(stats_p99,
                        os.path.join(seed_dir, "absmax_raw_C_p99_clip.npz"))
        ...
        print("    C-variant: nm_thres_cal ...")
        kv_absmax_arrs = [
            arr for s in stats_baseline.values() if s.sublayer in ["K", "V"]
            for arr in s._absmax_buf if len(arr) > 0
        ]
        if kv_absmax_arrs:
            ...
            stats_nt, handles_nt = register_hooks(model_nt, mask_buf_nt,
                                                  ...
                                                  store_sweep=False)
            run_diagnostic(model_nt, loader, n_step=N_STEP, seed=seed, desc=f"C-nm-s{seed}")
            for h in handles_nt:
                h.remove()
            del model_nt
            torch.cuda.empty_cache()
            ...
            save_absmax_npz(stats_nt,
                            os.path.join(seed_dir, "absmax_raw_C_nm_thres_cal.npz"))
            ...
            print("    [WARN] No K/V absmax — skipping nm_thres_cal")
        ...
        stats_sr, handles_sr = register_hooks(model_sr, mask_buf_sr, inp_res=inp_res_7,
                                               store_sweep=False)
        run_diagnostic(model_sr, loader, n_step=N_STEP, seed=seed, desc=f"C-sr-s{seed}")
        for h in handles_sr:
            h.remove()
        del model_sr
        torch.cuda.empty_cache()
        ...
        save_absmax_npz(stats_sr,
                        os.path.join(seed_dir, "absmax_raw_C_sto_round.npz"))
        ...
        df_c_steps = pd.DataFrame(all_c_step_records)
        df_c_steps["seed"] = seed
        df_c_steps.to_csv(CSV_C_STEPS, index=False)
        print(f"  → {CSV_C_STEPS} ({len(all_c_step_records)} rows)")

        figure_C(df_c, seed=seed, output_path=FIG_C)

    # ------------------------------------------------------------------ #
    # [5.5] Figure D — Layerwise Mixed-Precision Validation              #
    # ------------------------------------------------------------------ #
    if "D" in run_figures:
        high_dac = args.layerwise_high_bits
        high_adc = args.layerwise_high_adc_bits or args.layerwise_high_bits

        print(f"\n  [5.5] Figure D: layerwise mixed-precision "
              f"(policy={args.layerwise_policy}, high={high_dac}b/{high_adc}b) ...")

        tmp_model = AutoModelForQuestionAnswering.from_pretrained("bert-base-uncased")
        high_names = select_high_modules(tmp_model, stats_baseline,
                                          args.layerwise_policy, args)
        del tmp_model
        ...
        model_fh = create_model(dac_bits=high_dac, adc_bits=high_adc)
        ...
        figure_D(df_d, high_names, seed=seed, output_path=FIG_D)
...
def main():
    ...
    for seed in SEEDS_LIST:
        run_single_seed(seed, tokenizer, run_figures)
        gc.collect()
        torch.cuda.empty_cache()
    ...
    # ------------------------------------------------------------------ #
    agg_dir = os.path.join(OUT_DIR, "squad", "aggregate")
    ...
    if all_dfs:
        ...
        agg_csv = os.path.join(agg_dir, "metrics_squad_seed_summary.csv")
        agg_df.to_csv(agg_csv, index=False)
        print(f"\n[Aggregate] → {agg_csv} ({len(agg_df)} rows)")

    # Final summary
    print(f"\n{'='*60}")
    print(f"  DONE — SQuAD Backward-Underflow Diagnostic")
    print(f"{'='*60}")
    print(f"  Seeds:   {SEEDS_LIST}")
    print(f"  Figures: {run_figures}")
    print(f"  Output:  {OUT_DIR}/squad/")
    for seed in SEEDS_LIST:
        seed_dir = os.path.join(OUT_DIR, "squad", f"seed_{seed}")
        if os.path.isdir(seed_dir):
            ...
            print(f"  seed_{seed}: {n_csv} CSVs, {n_png} PNGs, {n_npz} NPZs")
    agg_dir = os.path.join(OUT_DIR, "squad", "aggregate")
    if os.path.isdir(agg_dir):
        print(f"  aggregate/: {', '.join(sorted(os.listdir(agg_dir)))}")
