#!/usr/bin/env python3
"""Re-run failed decay configurations with nearby TE hyperparameters (v2).

Failed configs and nearby HP choices:
- rank=4, te=10: Try te=1 and te=50 HP
- rank=8, te=10: Try te=1 HP (te=50 also failed)
- rank=64, te=100: Try te=50 and te=500 HP

All runs use lifetime=46505 (sixt1c default).
Settings identical to sweep_softbounds_lifetime.py
"""
...
# Failed configurations with NEARBY TE hyperparameters from original HYPERPARAMETERS dict
FAILED_CONFIGS = [
    # rank=4, te=10: Try te=1 HP
    {'rank': 4, 'te': 10, 'lr': 0.089054, 'tlr': 0.001277, 'source': 'te=1'},
    # rank=4, te=10: Try te=50 HP
    {'rank': 4, 'te': 10, 'lr': 0.674107, 'tlr': 0.011775, 'source': 'te=50'},
    # rank=8, te=10: Try te=1 HP
    {'rank': 8, 'te': 10, 'lr': 0.089054, 'tlr': 0.001277, 'source': 'te=1'},
    # rank=64, te=100: Try te=50 HP
    {'rank': 64, 'te': 100, 'lr': 0.008838, 'tlr': 0.008245, 'source': 'te=50'},
    # rank=64, te=100: Try te=500 HP
    {'rank': 64, 'te': 100, 'lr': 0.006316, 'tlr': 0.53314, 'source': 'te=500'},
]

# SoftBounds config (no noise) - identical to sweep_softbounds_lifetime.py
SOFTBOUNDS_CONFIG = {
...
def create_model(rank: int, te: int, lifetime: float, lr: float, tlr: float):
    """Create LRTT model - identical to sweep_softbounds_lifetime.py."""
...
def run_training(config, run_id, train_loader, val_loader, use_wandb):
    """Run single training - identical logic to sweep_softbounds_lifetime.py."""
    rank = config['rank']
    te = config['te']
    lr = config['lr']
    tlr = config['tlr']
    ...
    run_name = f"rerun2_r{rank}_te{te}_from{source}_run{run_id}"
    ...
    if use_wandb:
        wandb.init(
            ...
            tags=[f"rank={rank}", f"te={te}", f"lifetime={LIFETIME}",
                  "decay", "rerun_v2", "bias=True", f"source={source}"],
            config={"rank": rank, "te": te, "lifetime": LIFETIME,
                    "lr": lr, "tlr": tlr, "run": run_id,
                    "mode": "decay", "rerun": True, "source": source},
            reinit=True
        )
...
def main():
    print("="*70)
    print("Re-running Failed Decay Configurations (v2)")
    print("Using nearby TE hyperparameters from original HYPERPARAMETERS dict")
    print("="*70)
