#!/usr/bin/env python3
"""LRTT-v1 (decay / lora / onehot) AB-tile AF/noise cell-grid sweep, rank=8.

Mirrors the cell structure of hp_search_v2_rank8_b_af_noise.py so that the
(up_down × noise_ratio) envelope (= max over cells) gives an HP-optimized
robustness map per (AF, noise) point — directly comparable to v2.

DIFFERENCE FROM v2 SCRIPT:
  v1 has A and B as identical 6T1C LinearStepDevice (matches the original
  lrtt_v2_mnist_6t1c.py setup `unit_cell_devices=[ab, ab, c]`). So we sweep
  the SAME (up_down, noise_ratio) on BOTH A and B in lockstep, not just B.
  C-tile remains idealized.
...
LRTT-v1 backbone:
  ...
  - reinit_mode      = "decay"
  - decay_factor     = 1.0
  - te (transfer_every) = 10  (v1-decay-best for rank=8 from
        mnist_sweep_analysis/lrtt_sweep_best_configs.json :: decay_sweep)

Cell axes (6 cells = 2 × 3 × 1):
  LR_GRID        = {1.0, 0.1}
  TLR_GRID       = {0.01, 0.001, 0.0001}       # v1 best tlr=7.91e-4 sits here
...
Total: 12 × 25 = 300 trials, ~4–5 h on 1 GPU.

Usage:
  python hp_search_v1_decay_b_af_noise.py
  python hp_search_v1_decay_b_af_noise.py --smoke
"""
...
RANK = 8
TE = 10  # v1-decay-best for rank=8

# Cell axes — 12 cells total
LR_GRID = [1.0, 0.1]
TLR_GRID = [0.01, 0.001, 0.0001]
LIFETIME_GRID = [1000]  # fixed at 1000 per user request
...
# ---------------------------------------------------------------------------
# Devices — A and B share the same swept (up_down, noise_ratio) (v1 6T1C ab)
# ---------------------------------------------------------------------------
def _make_ab_device(ab_lifetime_param, up_down, noise_ratio):
    """6T1C-style LinearStepDevice for both A and B tiles in v1.
    `up_down` and `noise_ratio` are swept together for A and B."""
...
def create_model(rank, te, tlr, lifetime_phys, ab_up_down, ab_noise_ratio):
    """Build the LRTT-v1 (decay / lora / onehot) analog MLP.
    ...
    device_config.update_mode = "lora"          # ← v1
    device_config.transfer_method = "onehot"    # ← v1 (default but explicit)
...
# ---------------------------------------------------------------------------
# Sweep driver — one cell = one (lr, tlr, lifetime) triple, sweeps 5×5 B-tile
# ---------------------------------------------------------------------------
def sweep_cell(lr, tlr, lifetime_phys, output_dir):
    ...
    record = {
        "method": "lrtt_v1_decay_lora_onehot",
        "rank": RANK, "te": TE,
...
def main():
    ...
    if args.smoke:
        print("=== smoke trial ===")
        acc, hist = run_trial(
            RANK, TE, lr=0.187401, tlr=0.000791, lifetime_phys=46505,
            ab_up_down=2.0, ab_noise_ratio=1.0, epochs=2, smoke=True,
        )
        print(f"  acc={acc:.2f}%  hist={hist}")
        return

    output_dir = args.out or "/root/LRTT/results/hp_search_v1_decay_rank8_b_af_noise"
    ...
    print("=" * 74)
    print(f"LRTT-v1 (decay / lora / onehot)  B-tile AF/noise sweep — rank={RANK}")
    ...
    for lt in LIFETIME_GRID:
        ...
        header = "    lr\\tlr   " + "".join(f"  {t:>9.5f}" for t in TLR_GRID)
        print(header)
        for lr in LR_GRID:
            row = f"  {lr:>8.2f}  "
            for tlr in TLR_GRID:
