#!/bin/bash
# Recovery: only re-run FAILED experiments (skip D1 det which already completed)
# Fix: bs=16 acc=3 (was bs=24 acc=2 → OOM), --diag-layer-set 0,5,11
set -uo pipefail
...
# ============================================================
# D1: Sub-pulse mapping — 128 steps (STOCHASTIC ONLY, det already done)
# ============================================================
D1_DIR="results/paper/diag_D1_subpulse"
mkdir -p "$D1_DIR"

D1_ARGS="$COMMON --max-steps 128 --batch-size 16 --grad-accum-steps 3 --diag-at-steps 1,8,16,24,32,48,64,80,96,128 --log-every 20"

echo "============================================================"
echo "  D1 Recovery: Stochastic only (128 steps, layers 0,5,11)"
echo "  bs=16 acc=3 (effective batch=48)"
echo "  Start: $(date)"
echo "============================================================"
...
echo ""
echo "=== D1 Recovery complete: $(date) ==="
echo ""

# ============================================================
# D2: Carry-path — 1024 steps (ALL: baselines + TTv1)
# ============================================================
...
echo "============================================================"
echo "  D2: Carry-path (1024 steps, layers 0,5,11)"
echo "  Start: $(date)"
echo "============================================================"

# Single RPU baselines {8,10,12,14}b
for BITS in 8 10 12 14; do
    run_one D2 "single_rpu_stoch_${BITS}b" $COMMON --max-steps 1024 --batch-size 16 --grad-accum-steps 3 $D2_DIAG \
...
echo ""
echo "============================================================"
echo "  Recovery ALL COMPLETE: $(date)"
echo "============================================================"
