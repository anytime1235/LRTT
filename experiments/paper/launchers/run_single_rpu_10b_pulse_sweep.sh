#!/bin/bash
# Single RPU (ConstantStep 10-bit) — Stochastic → Deterministic 순차 실행
# TTv1 bit sweep과 동일한 학습 하이퍼파라미터 사용
#
# 공통 조건 (bit_sweep_summary.json과 동일):
#   model: bert-base-uncased, task: SQuAD v1.1
#   n_bits: 10, epochs: 4, batch_size: 16, grad_accum_steps: 3 (eff=48)
...
RESULTS_DIR="${RESULTS_DIR:-results/paper/single_rpu_10b_pulse_sweep}"
PYTHON="${PYTHON:-/root/.venv310/bin/python}"

cd "$SCRIPT_DIR"
mkdir -p "$RESULTS_DIR"

echo "=== Single RPU 10-bit Pulse Type Sweep ==="
echo "Results: $RESULTS_DIR"
echo "Start: $(date)"

run() {
    local PULSE=$1
    local TAG="single_rpu_10b_${PULSE}"
    echo ""
    echo "============================================"
    echo "  $TAG  (10-bit, pulse=${PULSE})"
    echo "============================================"
...
# 순차 실행: stochastic → deterministic
run stochastic
run deterministic
...
# Print summary
for PULSE in stochastic deterministic; do
    TAG="single_rpu_10b_${PULSE}"
    DIR="$RESULTS_DIR/$TAG"
