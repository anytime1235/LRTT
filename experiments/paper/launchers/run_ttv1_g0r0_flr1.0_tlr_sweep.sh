#!/bin/bash
# gamma=0, reset=0, fast_lr=1.0, epochs=4: transfer_lr sweep
# transfer_lr: [0.001, 0.01, 0.1, 1.0]
# → 4 conditions (~4h each ≈ 16h total)
#
# Base settings:
#   method=ttv1, seed=42, n_bits=14, n_bits_slow=10
#   batch=16, grad_accum=3, transfer_every=3, uim=true, scale_transfer_lr=false
#   ln_lr=0.003, fast_lr=1.0

set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
RESULTS_DIR="${RESULTS_DIR:-results/paper/g0r0_flr1.0_tlr_sweep}"
...
echo "=== gamma=0, reset=0, fast_lr=1.0: transfer_lr sweep (${EPOCHS}ep) ==="
echo "Results: $RESULTS_DIR"
echo "Start: $(date)"

TRANSFER_LRS=(0.001 0.01 0.1 1.0)
TOTAL=${#TRANSFER_LRS[@]}
COUNT=0
...
base = os.environ.get("RESULTS_DIR", "results/paper/g0r0_flr1.0_tlr_sweep")
transfer_lrs = [0.001, 0.01, 0.1, 1.0]
