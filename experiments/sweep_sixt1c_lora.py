#!/usr/bin/env python
"""
Sixt1c (6T1C) LoRA Hyperparameter Sweep Script.

Sweeps lora_alpha and analog_lr for different LoRA ranks on GLUE tasks.
...
Usage:
    ...
    python sweep_sixt1c_lora.py --lora_rank 1

    # Run sweep for rank 4
    python sweep_sixt1c_lora.py --lora_rank 4

    # Run sweep for rank 8
    python sweep_sixt1c_lora.py --lora_rank 8

    # Run with nohup (background)
    nohup python sweep_sixt1c_lora.py --lora_rank 1 > /data/AIMC_LoRA_results/sweep_sixt1c_lora_r1.log 2>&1 &
"""
...
# GLUE tasks to sweep (excluding WNLI due to small size)
...
def run_single_experiment(
    ...
    sweep_dir = RESULTS_DIR / "sweep_sixt1c_lora" / f"rank{lora_rank}" / task_name
    output_dir = sweep_dir / f"trial_{trial_idx:03d}"
    output_dir.mkdir(parents=True, exist_ok=True)

    # Build run name
    run_name = f"sweep_sixt1c_r{lora_rank}_{task_name}_t{trial_idx:03d}_alpha{lora_alpha:.4f}_lr{analog_lr:.2e}"
...
def run_sweep(
    ...
    print(f"\n{'='*60}")
    print(f"SIXT1C LoRA HYPERPARAMETER SWEEP")
    print(f"{'='*60}")
    print(f"LoRA Rank: {lora_rank}")
    print(f"Tasks: {tasks}")
    ...
    # Save sweep plan
    sweep_plan_dir = RESULTS_DIR / "sweep_sixt1c_lora" / f"rank{lora_rank}"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run Sixt1c LoRA hyperparameter sweep on GLUE tasks"
    )
    parser.add_argument(
        "--lora_rank",
        type=int,
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=None,
        help="Specific GLUE tasks to run (default: all 8 tasks)"
    )
    parser.add_argument(
        "--num_trials",
        type=int,
