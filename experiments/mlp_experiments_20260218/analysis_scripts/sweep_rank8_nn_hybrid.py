#!/usr/bin/env python3
"""Rank=8 Transfer Every Sweep - HYBRID mode (NEAREST NEIGHBOR).

Uses nearest neighbor strategy for lr/tlr selection.
Fixed rank=8, sweep 28 NEW transfer_every values with 3 trials each (±30%).
Excludes already tested: [1, 10, 50, 100, 500, 1000]
...
def create_model(te: int, lifetime: float, lr: float, tlr: float):
    ...
    # Disable output noise on all tiles (A, B, C)
    rpu_config.forward.out_noise = 0.0
    rpu_config.backward.out_noise = 0.0

    # Weight scaling for C tile (same as train_analog.py)
...
def main():
    # Load configs
    with open('/root/rank8_nn_sweep_configs.json', 'r') as f:
        sweep_config = json.load(f)

    results_dir = "/root/results/rank8_nn_hybrid"
    os.makedirs(results_dir, exist_ok=True)

    hybrid_configs = sweep_config['hybrid']['configs']
    total_configs = len(hybrid_configs)

    print("=" * 70)
    print("RANK=8 TRANSFER_EVERY SWEEP - HYBRID MODE (NEAREST NEIGHBOR)")
    print("=" * 70)
    print(f"Strategy: Nearest Neighbor (±30% variation)")
    print(f"Excluded (already tested): {sweep_config['metadata']['existing_tes_excluded']}")
    print(f"Total NEW TE values: {total_configs}")
    print(f"TE range: 2 - 10000")
    ...
    for idx, config in enumerate(hybrid_configs, 1):
        te = config['te']
        trials = config['trials']

        print(f"[{idx}/{total_configs}] TE={te}")
        print(f"  Base: lr={config['lr_base']:.5f}, tlr={config['tlr_base']:.5f}")
        print("-" * 50)
    ...
    # Summary
    print("\n" + "=" * 70)
    print("SWEEP COMPLETE - HYBRID MODE")
    print("=" * 70)
