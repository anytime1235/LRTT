#!/usr/bin/env python3
"""Generate rank=8 transfer_every sweep configs (EXTENDED to 10000)."""
...
print("=" * 70)
print("RANK=8 TRANSFER_EVERY SWEEP (EXTENDED TO 10000)")
print("=" * 70)

# Generate 20 TE values in log-scale from 1 to 10000
te_values = np.logspace(np.log10(1), np.log10(10000), 20, dtype=int)
te_values = np.unique(te_values)  # Remove duplicates

# If we don't have exactly 20, manually create a better distribution
if len(te_values) != 20:
    te_values = np.array([
        1, 2, 4, 8, 15, 30, 60, 100, 180, 320,
        560, 1000, 1800, 3200, 4600, 6200, 7500, 8500, 9500, 10000
    ], dtype=int)

print(f"\n20 Transfer Every values: {te_values.tolist()}")
print(f"Range: {te_values[0]} - {te_values[-1]}")
print(f"Total data points: {len(te_values)}")
...
def smart_interpolate(known_data, te_new):
    ...
    for te in te_new:
        if te <= tes[0]:
            ...
            # For TE > 1000, clamp to last value but slightly adjust
            # This is safer than wild extrapolation
            lr = lrs[-1]
            tlr = tlrs[-1]
        else:
...
output_file = '/root/rank8_te20_sweep_configs.json'
...
print(f"Total experiments: {len(te_values) * 3 * 2} runs")
