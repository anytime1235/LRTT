...
def get_completed_runs():
    """Get set of completed runs from wandb for resume support."""
    if not WANDB_AVAILABLE:
        return set()
    try:
        api = wandb.Api()
        runs = api.runs("lrtt-softbounds-sweep")
        completed = set()
        for run in runs:
            if run.state == 'finished':
                ...
                lt = cfg.get('lifetime')
                run_id = cfg.get('run')
                if rank is not None and te is not None and lt is not None and run_id is not None:
                    key = f"r{rank}_te{te}_lt{lt}_run{run_id}"
                    completed.add(key)
        return completed
    except Exception as e:
        print(f"Warning: Could not fetch completed runs: {e}")
        return set()
...
def main():
    ...
    print(f"Sweep: {total_configs} configs x {RUNS_PER_CONFIG} runs = {total_runs} total")

    # Resume support: get completed runs from wandb
    print("Checking wandb for completed runs...")
    ...
    sweep_group = "softbounds_sweep_resume"  # Fixed group name for continuity
    ...
    for lifetime in LIFETIMES:
        for rank in RANKS:
            for te in TES:
                ...
                run_accs = []
                runs_skipped_this_config = 0

                for run in range(RUNS_PER_CONFIG):
                    run_key = f"r{rank}_te{te}_lt{lifetime}_run{run}"

                    # Skip if already completed
                    if run_key in completed_runs:
                        runs_skipped_this_config += 1
                        skipped_runs += 1
                        continue
                    ...
                    if use_wandb:
                        wandb.init(
                            ...
                            tags=[f"rank={rank}", f"te={te}", f"lifetime={lifetime}",
                                  "SoftBounds", "decay", "bias=True", "resume"],
                ...
                # Handle case where all runs were skipped
                if runs_skipped_this_config == RUNS_PER_CONFIG:
                    print(f"[{config_idx}/{total_configs}] r{rank}_te{te}_lt{lifetime}: SKIPPED (all {RUNS_PER_CONFIG} runs done)")
                    continue

                # Handle case where some runs were skipped
                if runs_skipped_this_config > 0:
                    print(f"[{config_idx}/{total_configs}] r{rank}_te{te}_lt{lifetime}: {runs_skipped_this_config} runs skipped, {len(run_accs)} new runs")
    ...
    # Summary
    print(f"\n=== Resume Summary ===")
    print(f"Total skipped (already done): {skipped_runs}")
    print(f"New runs completed: {total_runs - skipped_runs - (total_runs - skipped_runs - len(all_results) * RUNS_PER_CONFIG)}")

    if all_results:
        global_best = max(all_results, key=lambda x: x['mean_acc'])
        print(f"BEST (this session): r{global_best['rank']}_te{global_best['te']}_lt{global_best['lifetime']} = {global_best['mean_acc']:.2f}%")
