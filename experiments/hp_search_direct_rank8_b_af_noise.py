#!/usr/bin/env python3
"""Direct-update baseline (SingleRPUConfig + 6T1C) AF/noise sweep, rank=8.

The simplest analog hardware baseline: a single 6T1C LinearStepDevice trained
directly with SGD (no compound/transfer/LoRA/rank decomposition). This is the
worst-case point that LRTT-v1, LRTT-v2, and TikiTaka v1 should beat.

Architecture:
  - SingleRPUConfig with one 6T1C LinearStepDevice tile (sweep target)
  - No transfer compound, no slow tile, no rank-r structure

Hyperparameters are FIXED — no HP search:
  - lr        = 0.1                    (matches tikitaka)
  - lifetime_phys = 1000               (per user request)
  - desired_bl = 31
  - mapping.weight_scaling_omega=0.6 (matches LRTT v1/v2 sweep convention)

Sweep variables (applied to the SINGLE 6T1C tile):
...
Output: /root/LRTT/results/hp_search_direct_rank8_b_af_noise/

Usage:
  python hp_search_direct_rank8_b_af_noise.py
  python hp_search_direct_rank8_b_af_noise.py --smoke
"""
...
from aihwkit.simulator.configs import (
    FloatingPointRPUConfig, SingleRPUConfig,
)
...
# Sweep grid (applied to the single 6T1C tile)
...
# ---------------------------------------------------------------------------
# Device — single 6T1C tile (same recipe as LRTT A/B and tikitaka Fast)
# ---------------------------------------------------------------------------
def _make_6t1c_device(lifetime_param, ab_up_down, ab_noise_ratio):
...
def create_direct_rpu(ab_up_down, ab_noise_ratio):
    """Direct-update RPU: SingleRPUConfig with a single 6T1C LinearStepDevice."""
    lifetime_param = _ab_lifetime_param(LIFETIME_PHYS)
    device = _make_6t1c_device(lifetime_param, ab_up_down, ab_noise_ratio)
...
def create_model(ab_up_down, ab_noise_ratio):
    rpu_config = create_direct_rpu(ab_up_down, ab_noise_ratio)
...
def main():
    ...
    output_dir = args.out or "/root/LRTT/results/hp_search_direct_rank8_b_af_noise"
    os.makedirs(output_dir, exist_ok=True)
    cell_path = f"{output_dir}/cell_lr{LR}_lt{LIFETIME_PHYS}.json"

    print("=" * 74)
    print("Direct-update baseline (SingleRPUConfig + 6T1C)  AF/noise sweep")
    print(f"Fixed: lr={LR}  lifetime_phys={LIFETIME_PHYS}  desired_bl={DESIRED_BL}")
    print(f"Sweep: ab_up_down ∈ {AB_UP_DOWN_GRID} × "
          f"ab_noise_ratio ∈ {AB_NOISE_RATIO_GRID}  (single 6T1C tile)")
    ...
    record = {
        "method": "direct_update_singleRPU_6t1c",
        "rank": None,
        "lr": LR, "lifetime_phys": LIFETIME_PHYS, "desired_bl": DESIRED_BL,
