#!/usr/bin/env python3
"""SRA-LRTT-v2 C-tile sweep with per-cell Optuna TPE search (~12 h total).

C-tile sweep on the Tiki-Taka / LRTT canonical AF × UPDATE-NOISE axes.
...
Fixed:
    ...
    AB tile = nominal 6T1C LinearStepDevice (mult_noise=False)
...
Run on its own GPU; pair with sweep_lrtt_v2_sra_abtile_tpe_12h.py on a 2nd GPU.

Usage:
    python sweep_lrtt_v2_sra_ctile_tpe_12h.py
    python sweep_lrtt_v2_sra_ctile_tpe_12h.py --trials_per_cell 8 --include_baseline
    python sweep_lrtt_v2_sra_ctile_tpe_12h.py --output_dir results/sra_ctile_tpe
"""
...
def make_c_device_6t1c_8bit_swept(*, af_ratio: float, update_noise_ratio: float):
    ...
    Held FIXED at 6T1C nominal (NOT scaled by either sweep axis):
        ...
        dw_min_dtod=0.1, up_down_dtod=0.01, w_*_dtod=0.05, gamma_*_dtod=0.05.

    Only dw_min is pinned at 8 bits (= 2/2^8 = 0.0078125).
    """
    return LinearStepDevice(
...
def build_model(*, transfer_lr: float, sigma_A: float,
    ...
    a = make_ab_device_nominal()
    b = make_ab_device_nominal()
    c = make_c_device_6t1c_8bit_swept(
...
def run_cell(*, af_ratio, update_noise_ratio,
    ...
    Returns (best_value, best_params, n_completed, n_pruned)."""
    study_name = f"sra_ctile_AF{af_ratio:g}_UNR{update_noise_ratio:g}"
...
def main():
    ...
    parser.add_argument("--include_baseline", action="store_true",
                        help="Include the (0,0) baseline cell.")
    parser.add_argument("--output_dir", default="results/sra_ctile_tpe_12h")
    ...
    cells = []
    for af_ratio in AF_RATIO_GRID:
        for update_noise_ratio in UPDATE_NOISE_RATIO_GRID:
    ...
    print("=" * 72)
    print("SRA-LRTT-v2 C-TILE PER-CELL TPE SEARCH (~12 h budget)")
    print("=" * 72)
    ...
    print(f"          AB = nominal 6T1C, C-tile precision = {C_BITS}b "
          f"(dw_min={C_DW_MIN:.6f})")
    print(f"  Outer grid (Tiki-Taka / LRTT canonical AF × UNR axes):")
    print(f"    AF ratio          ∈ {AF_RATIO_GRID}  → up_down = "
          f"{BASE_AF_TO_UP_DOWN}·AF")
    print(f"    Update-noise ratio∈ {UPDATE_NOISE_RATIO_GRID}  → dw_min_std = "
    ...
    for ci, cell in enumerate(cells, 1):
        ...
        print(f"\n[{ci}/{len(cells)}] cell: AF={af_ratio:g} "
    ...
    # Final heatmap
    print("\n" + "=" * 72)
    print("HEATMAP: SRA-LRTT-v2 C-tile per-cell best val_acc (%)")
    print("=" * 72)
    label_corner = "UNR\\AF"
    header = f"{label_corner:>8s}" + "".join(f"  AF={a:<5g}" for a in AF_RATIO_GRID)
    lines = [header, "-" * len(header)]
    for unr in UPDATE_NOISE_RATIO_GRID:
        row = f"{unr:>8g}"
        for af in AF_RATIO_GRID:
