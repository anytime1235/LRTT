...
# coding=utf-8
"""LRTT Bayesian Optimization Sweep with WandB Logging.

6T1C with SoftBounds C tile and onehot transfer.
...
def create_sixt1c_ab_softbounds_config(
    rank: int,
    transfer_every: int,
    transfer_lr: float,
    lora_alpha: float,
    ...
    """Create LRTT config with 6T1C A/B + SoftBounds C + onehot transfer."""
    TAU_SEC = 46505.0
    ...
    sixt1c_device = LinearStepDevice(
        ...
        lifetime=lifetime, lifetime_dtod=0.1 if include_retention else 0.0,
...
def create_lrtt_model(
    ...
    rpu_config = create_sixt1c_ab_softbounds_config(
...
def run_trial(
    ...
    run = wandb.init(
        ...
        config={
            ...
            "batch_size": BATCH_SIZE, "forward_inject": False, "transfer_method": "onehot",
            "device_type": "sixt1c_ab_softbounds",
        },
        reinit=True,
    )
...
def main():
    ...
    print("=" * 80)
    print("LRTT Bayesian Optimization Sweep with WandB")
    print("sixt1c_ab_softbounds: 6T1C A/B + SoftBounds C + onehot transfer")
    ...
    print(f"       forward_inject=False, transfer_method=onehot")
    ...
    # CSV logger
    csv_path = os.path.join(OUTPUT_DIR, f'lrtt_bayesian_results_{timestamp}.csv')
    ...
    study = optuna.create_study(study_name=f"lrtt_{timestamp}", direction="maximize", sampler=sampler)
    ...
    # Save JSON summary
    json_path = os.path.join(OUTPUT_DIR, f'lrtt_bayesian_summary_{timestamp}.json')
